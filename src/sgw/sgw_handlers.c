  ...
  const itti_s11_create_session_request_t * const session_req_pP)
{
  ...
  if (new_endpoint_p == NULL) {
    OAILOG_ERROR (LOG_SPGW_APP, "Could not create new tunnel endpoint between S-GW and MME " "for S11 abstraction\n");
  ...
  if (s_plus_p_gw_eps_bearer_ctxt_info_p ) {
    ...
    if (session_req_pP->apn) {
      ...
      s_plus_p_gw_eps_bearer_ctxt_info_p->sgw_eps_bearer_context_information.pdn_connection.apn_in_use = strdup("NO APN");
    }
    ...
    free(s_plus_p_gw_eps_bearer_ctxt_info_p->sgw_eps_bearer_context_information.pdn_connection.apn_in_use);
  ...
  itti_sgi_create_end_point_response_t * const resp_pP)
{
  ...
  OAILOG_DEBUG (LOG_SPGW_APP,
    "Tx CREATE-SESSION-RESPONSE SPGW -> TASK_MME, S11 MME teid %u S11 S-GW teid %u S1U teid %u S1U addr 0x%x EPS bearer id %u status %d\n",
  ...
  const Gtpv1uCreateTunnelResp * const endpoint_created_pP)
{
  ...
  if (HASH_TABLE_OK == hash_rc) {
    ...
    case IPv4:
      ...
      if (!pco_ids.ci_ipv4_address_allocation_via_dhcpv4) {
        if (0 == allocate_ue_ipv4_address(imsi, &inaddr)) {
          increment_counter ("ue_pdn_connection", 1, 2, "pdn_type", "ipv4", "result", "success");
          IN_ADDR_TO_BUFFER (inaddr, sgi_create_endpoint_resp.paa.ipv4_address);
          sgi_create_endpoint_resp.status = SGI_STATUS_OK;
        } else {
          increment_counter ("ue_pdn_connection", 1, 2, "pdn_type", "ipv4", "result", "failure");
    ...
    case IPv6:
          increment_counter ("ue_pdn_connection", 1, 2, "pdn_type", "ipv4v6", "result", "failure");
    ...
    case IPv4_AND_v6:
      if (0 == allocate_ue_ipv4_address(imsi, &inaddr)) {
        increment_counter ("ue_pdn_connection", 1, 2, "pdn_type", "ipv4v6", "result", "success");
        IN_ADDR_TO_BUFFER (inaddr, sgi_create_endpoint_resp.paa.ipv4_address);
        sgi_create_endpoint_resp.status = SGI_STATUS_OK;
      } else {
        increment_counter ("ue_pdn_connection", 1, 2, "pdn_type", "ipv4v6", "result", "failure");
  ...
  const itti_sgi_update_end_point_response_t * const resp_pP)
{
  ...
  if ((HASH_TABLE_OK == hash_rc) && (HASH_TABLE_OK == hash_rc2)) {
    ...
    if ((hash_rc == HASH_TABLE_KEY_NOT_EXISTS) || (hash_rc == HASH_TABLE_BAD_PARAMETER_HASHTABLE)) {
      ...
      BUFFER_TO_IN_ADDR(eps_bearer_entry_p->enb_ip_address_S1u.address.ipv4_address, enb);

      struct in_addr ue = {.s_addr = 0};
      BUFFER_TO_IN_ADDR(eps_bearer_entry_p->paa.ipv4_address, ue);

      Imsi_t imsi = new_bearer_ctxt_info_p->sgw_eps_bearer_context_information.imsi;
      rv = gtp_tunnel_ops->add_tunnel(ue, enb, eps_bearer_entry_p->s_gw_teid_S1u_S12_S4_up, eps_bearer_entry_p->enb_teid_S1u, imsi);
      if (rv < 0) {
  ...
  const itti_sgi_delete_end_point_request_t * const resp_pP)
{
  ...
  if (HASH_TABLE_OK == hash_rc) {
    ...
    if ((hash_rc == HASH_TABLE_KEY_NOT_EXISTS) || (hash_rc == HASH_TABLE_BAD_PARAMETER_HASHTABLE)) {
      ...
      rv = gtp_tunnel_ops->del_tunnel(ue, eps_bearer_entry_p->s_gw_teid_S1u_S12_S4_up, eps_bearer_entry_p->enb_teid_S1u);
      if (rv < 0) {
  ...
  if (HASH_TABLE_OK == hash_rc) {
    if ((delete_session_req_pP->sender_fteid_for_cp.ipv4 ) && (delete_session_req_pP->sender_fteid_for_cp.ipv6 )) {
      ...
      if (delete_session_req_pP->teid != ctx_p->sgw_eps_bearer_context_information.mme_teid_S11) {
        ...
        OAILOG_ERROR (LOG_SPGW_APP, "Mismatch in MME Teid for CP\n");
      } else {
      ...
      sgw_cm_remove_eps_bearer_entry(
        ctx_p->sgw_eps_bearer_context_information.pdn_connection.sgw_eps_bearers,
...
//#pragma message  "TODO Here the release (sgw_handle_release_access_bearers_request)"
    /*
     * Release the tunnel so that in idle state, DL packets are not sent towards eNB.
     * This tunnel will be added again when UE moves back to connected mode.
     */
    hash_rc = hashtable_ts_get (ctx_p->sgw_eps_bearer_context_information.pdn_connection.sgw_eps_bearers,
                               release_access_bearers_req_pP->list_of_rabs.ebis[0],
                               (void **)&eps_bearer_entry_p);
    if (HASH_TABLE_OK == hash_rc) {
      OAILOG_DEBUG (LOG_SPGW_APP,"Handle S11_RELEASE_ACCESS_BEARERS_REQUEST: Release GTP-U Tunnel mapping in GTP-U Kernel module \n");
      // delete GTPv1-U tunnel
      struct in_addr ue = {.s_addr = 0};
      BUFFER_TO_IN_ADDR(eps_bearer_entry_p->paa.ipv4_address, ue);
      rv = gtp_tunnel_ops->del_tunnel(ue,
                  eps_bearer_entry_p->s_gw_teid_S1u_S12_S4_up,
