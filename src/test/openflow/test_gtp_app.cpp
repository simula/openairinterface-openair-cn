  ...
  virtual void SetUp() {
    ...
    controller = std::unique_ptr<OpenflowController>(
      new OpenflowController(
        ...
        false,
        messenger));
    controller->register_for_event(gtp_app,
                                   openflow::EVENT_ADD_GTP_TUNNEL);
    controller->register_for_event(gtp_app,
                                   openflow::EVENT_DELETE_GTP_TUNNEL);
  }
...
MATCHER_P(CheckInPort, port_num, "") {
  auto msg = static_cast<of13::FlowMod*>(&arg);
  auto in_port = static_cast<of13::InPort*>(msg->get_oxm_field(
    of13::OFPXMT_OFB_IN_PORT));
  return in_port->value() == port_num;
}

MATCHER_P(CheckTunnelId, tunnel_id, "") {
  auto msg = static_cast<of13::FlowMod*>(&arg);
  auto tun_field = static_cast<of13::TUNNELId*>(msg->get_oxm_field(
...
MATCHER_P(CheckIPv4Dst, ip, "") {
  auto msg = static_cast<of13::FlowMod*>(&arg);
  auto ipv4_field = static_cast<of13::IPv4Dst*>(msg->get_oxm_field(
    of13::OFPXMT_OFB_IPV4_DST));
  return ipv4_field->value().getIPv4() == ip.s_addr;
}

MATCHER_P(CheckEthType, eth_type, "") {
  auto msg = static_cast<of13::FlowMod*>(&arg);
  auto eth_type_field = static_cast<of13::EthType*>(msg->get_oxm_field(
    of13::OFPXMT_OFB_ETH_TYPE));
...
TEST_F(GTPApplicationTest, TestAddTunnel) {
  ...
  // Uplink
  EXPECT_CALL(*messenger, send_of_msg(
   AllOf(
     CheckTableId(0),
