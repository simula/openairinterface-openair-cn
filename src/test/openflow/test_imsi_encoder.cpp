...
TEST_P(IMSIEncoderTest, TestCompactExpand) {
  std::string imsi_test = IMSIEncoder::expand_imsi(
    IMSIEncoder::compact_imsi(GetParam()));
  ASSERT_STREQ(GetParam().c_str(), imsi_test.c_str());
}

INSTANTIATE_TEST_CASE_P(TestLeadingZeros,
                        IMSIEncoderTest,
                        Values("001010000000013", "011010000000013",
                               "111010000000013"));

INSTANTIATE_TEST_CASE_P(TestDifferentLengths,
                        IMSIEncoderTest,
