...
class Service303Test : public ::testing::Test {
  public:
    ...
    virtual void SetUp() {
      ...
      int rc = pthread_create(&server_thread, NULL, thread_init, (void*) &Service303Test::config);
      if (rc){
  ...
  private:
    static void setupClient() {
      ...
      char* name = bstr2cstr(config.name, (char) '?');
      std::string service_addr =
        magma::ServiceRegistrySingleton::Instance()->GetServiceAddrString(name);
      const std::shared_ptr<Channel> channel = CreateChannel(service_addr,
                                                             cred);
      free(name);
...
TEST_F(Service303Test, GetServiceInfo) {
  ...
  EXPECT_EQ(0, status);
  char* name = bstr2cstr(Service303Test::config.name, (char) '?');
  char* version = bstr2cstr(Service303Test::config.version, (char) '?');
...
TEST_F(Service303Test, TestCounters) {
  ...
  const MetricFamily& family = Service303Test::findFamily(metrics_container, "test_counter");
  EXPECT_EQ(family.name(), "test_counter");
  const io::prometheus::client::Counter& counter = family.metric().Get(0).counter();
...
TEST_F(Service303Test, TestGauges) {
  ...
  MetricsContainer metrics_container;
  int status = client.GetMetrics(&metrics_container);
  const MetricFamily* family = &Service303Test::findFamily(metrics_container, "test_gauge");
...
TEST_F(Service303Test, TestHistograms) {
  ...
  MetricsContainer metrics_container;
  int status = client.GetMetrics(&metrics_container);
  const MetricFamily* family = &Service303Test::findFamily(metrics_container, "test_hist");
  ...
  EXPECT_EQ(histogram->bucket().Get(0).upper_bound(), std::numeric_limits<double>::infinity());
  ...
  EXPECT_EQ(histogram->bucket().Get(3).upper_bound(), std::numeric_limits<double>::infinity());
...
TEST_F(Service303Test, TestTimingMetrics) {
  ...
  double start_time = Service303Test::findGauge(metrics_container,
    std::to_string(magma::MetricName::process_start_time_seconds));
  double uptime = Service303Test::findGauge(metrics_container,
    std::to_string(magma::MetricName::process_cpu_seconds_total));
  ...
  double new_start = Service303Test::findGauge(metrics_container,
    std::to_string(magma::MetricName::process_start_time_seconds));
  double new_uptime = Service303Test::findGauge(metrics_container,
    std::to_string(magma::MetricName::process_cpu_seconds_total));
...
TEST_F(Service303Test, TestMemoryMetrics) {
  ...
  MetricsContainer metrics_container;
  int status = client.GetMetrics(&metrics_container);
  double physical_mem = Service303Test::findGauge(metrics_container,
    std::to_string(magma::MetricName::process_resident_memory_bytes));
  double virtual_mem = Service303Test::findGauge(metrics_container,
    std::to_string(magma::MetricName::process_virtual_memory_bytes));
...
TEST_F(Service303Test, TestEnumConversions) {
  ...
  increment_counter("mme_new_association", 3, 2,
                    "result", "success",
                    "gateway", "1234");

  MetricsContainer metrics_container;
  int status = client.GetMetrics(&metrics_container);
  const MetricFamily& family = Service303Test::findFamily(
    metrics_container, "500");
