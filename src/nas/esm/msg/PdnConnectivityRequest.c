  ...
  uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case PDN_CONNECTIVITY_REQUEST_ESM_INFORMATION_TRANSFER_FLAG_IEI:
      if ((decoded_result = decode_esm_information_transfer_flag (&pdn_connectivity_request->esminformationtransferflag, PDN_CONNECTIVITY_REQUEST_ESM_INFORMATION_TRANSFER_FLAG_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
    ...
    case PDN_CONNECTIVITY_REQUEST_ACCESS_POINT_NAME_IEI:
      if ((decoded_result = decode_access_point_name_ie (&pdn_connectivity_request->accesspointname, true, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
    ...
    case PDN_CONNECTIVITY_REQUEST_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
      if ((decoded_result = decode_protocol_configuration_options_ie (&pdn_connectivity_request->protocolconfigurationoptions, true, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
...
int
encode_pdn_connectivity_request (
  pdn_connectivity_request_msg * pdn_connectivity_request,
  uint8_t * buffer,
  uint32_t len)
{
  ...
  *(buffer + encoded) = ((encode_u8_pdn_type (&pdn_connectivity_request->pdntype) & 0x0f) << 4) | (encode_u8_request_type (&pdn_connectivity_request->requesttype) & 0x0f);
  ...
  if ((pdn_connectivity_request->presencemask & PDN_CONNECTIVITY_REQUEST_ESM_INFORMATION_TRANSFER_FLAG_PRESENT)
    ...
    if ((encode_result = encode_esm_information_transfer_flag (&pdn_connectivity_request->esminformationtransferflag, PDN_CONNECTIVITY_REQUEST_ESM_INFORMATION_TRANSFER_FLAG_IEI, buffer + encoded, len - encoded)) < 0)
  ...
  if ((pdn_connectivity_request->presencemask & PDN_CONNECTIVITY_REQUEST_ACCESS_POINT_NAME_PRESENT)
    ...
    if ((encode_result = encode_access_point_name_ie (pdn_connectivity_request->accesspointname, true, buffer + encoded, len - encoded)) < 0)
      // Return in case of error
      return encode_result;
    else
      encoded += encode_result;
  ...
  if ((pdn_connectivity_request->presencemask & PDN_CONNECTIVITY_REQUEST_PROTOCOL_CONFIGURATION_OPTIONS_PRESENT)
    ...
    if ((encode_result = encode_protocol_configuration_options_ie (&pdn_connectivity_request->protocolconfigurationoptions, true, buffer + encoded, len - encoded)) < 0)
      // Return in case of error
      return encode_result;
    else
      encoded += encode_result;
