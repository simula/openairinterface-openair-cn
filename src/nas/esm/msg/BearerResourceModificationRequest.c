...
int
decode_bearer_resource_modification_request (
  ...
  uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case BEARER_RESOURCE_MODIFICATION_REQUEST_REQUIRED_TRAFFIC_FLOW_QOS_IEI:
      if ((decoded_result = decode_eps_quality_of_service (&bearer_resource_modification_request->requiredtrafficflowqos, BEARER_RESOURCE_MODIFICATION_REQUEST_REQUIRED_TRAFFIC_FLOW_QOS_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
    ...
    case BEARER_RESOURCE_MODIFICATION_REQUEST_ESM_CAUSE_IEI:
      if ((decoded_result = decode_esm_cause (&bearer_resource_modification_request->esmcause, BEARER_RESOURCE_MODIFICATION_REQUEST_ESM_CAUSE_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
...
int
encode_bearer_resource_modification_request (
  bearer_resource_modification_request_msg * bearer_resource_modification_request,
  uint8_t * buffer,
  uint32_t len)
{
  ...
  *(buffer + encoded) = ((encode_u8_linked_eps_bearer_identity (&bearer_resource_modification_request->epsbeareridentityforpacketfilter) & 0x0f) << 4) | 0x00;
  ...
  if ((bearer_resource_modification_request->presencemask & BEARER_RESOURCE_MODIFICATION_REQUEST_REQUIRED_TRAFFIC_FLOW_QOS_PRESENT)
    ...
    if ((encode_result = encode_eps_quality_of_service (&bearer_resource_modification_request->requiredtrafficflowqos, BEARER_RESOURCE_MODIFICATION_REQUEST_REQUIRED_TRAFFIC_FLOW_QOS_IEI, buffer + encoded, len - encoded)) < 0)
  ...
  if ((bearer_resource_modification_request->presencemask & BEARER_RESOURCE_MODIFICATION_REQUEST_ESM_CAUSE_PRESENT)
    ...
    if ((encode_result = encode_esm_cause (&bearer_resource_modification_request->esmcause, BEARER_RESOURCE_MODIFICATION_REQUEST_ESM_CAUSE_IEI, buffer + encoded, len - encoded)) < 0)
      // Return in case of error
      return encode_result;
    else
      encoded += encode_result;
