  ...
  uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case BEARER_RESOURCE_MODIFICATION_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
       ...
       */
      bearer_resource_modification_reject->presencemask |= BEARER_RESOURCE_MODIFICATION_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_PRESENT;
      break;
...
int
encode_bearer_resource_modification_reject (
  bearer_resource_modification_reject_msg * bearer_resource_modification_reject,
  uint8_t * buffer,
  uint32_t len)
{
  ...
  if ((encode_result = encode_esm_cause (&bearer_resource_modification_reject->esmcause, 0, buffer + encoded, len - encoded)) < 0)      //Return in case of error
