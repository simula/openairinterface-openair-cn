  ...
  const_bstring auts)
{
  ...
  case EMM_CAUSE_SYNCH_FAILURE:
    ...
    if (EMM_AUTHENTICATION_SYNC_FAILURE_MAX > emm_ctx->auth_sync_fail_count) {
      ...
      nas_itti_auth_info_req(ue_id, emm_ctx->_imsi64, false, &emm_ctx->serving_network_id, MAX_EPS_AUTH_VECTORS,
                             &resync_param, emm_ctx->_imsi.length);
      ...
      increment_counter ("authentication_failure", 1, 1, "cause", "resync");
      increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "authentication_sync_failure");
  ...
  case EMM_CAUSE_MAC_FAILURE:
    ...
    if (!IS_EMM_CTXT_PRESENT_IMSI(emm_ctx)) { // VALID means received in IDENTITY RESPONSE
      ...
      emm_ctx->emm_cause = EMM_CAUSE_ILLEGAL_UE;
      increment_counter ("authentication_failure", 1, 1, "cause", "mac_failure");
      increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "authentication_mac_failure");
      // Do not accept the UE to attach to the network
      rc = _authentication_reject(data);
    }
    break;
  ...
  case EMM_CAUSE_NON_EPS_AUTH_UNACCEPTABLE:
    increment_counter ("authentication_failure", 1, 1, "cause", "amf_unacceptable");
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "authentication_amf_failure");
    emm_ctx->auth_sync_fail_count = 0;
    ...
    if (emm_ctx) {
      ...
      if (IS_EMM_CTXT_VALID_IMSI(emm_ctx)) { // VALID means received in IDENTITY RESPONSE
        if (emm_ctx->_imsi64 != emm_ctx->saved_imsi64) {
          nas_itti_auth_info_req (emm_ctx->ue_id, emm_ctx->_imsi64, false, &emm_ctx->serving_network_id,
