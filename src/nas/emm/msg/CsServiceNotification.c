  ...
  uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case CS_SERVICE_NOTIFICATION_CLI_IEI:
      if ((decoded_result = decode_cli (&cs_service_notification->cli, CS_SERVICE_NOTIFICATION_CLI_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
    ...
    case CS_SERVICE_NOTIFICATION_SS_CODE_IEI:
      if ((decoded_result = decode_ss_code (&cs_service_notification->sscode, CS_SERVICE_NOTIFICATION_SS_CODE_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
    ...
    case CS_SERVICE_NOTIFICATION_LCS_INDICATOR_IEI:
      if ((decoded_result = decode_lcs_indicator (&cs_service_notification->lcsindicator, CS_SERVICE_NOTIFICATION_LCS_INDICATOR_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
    ...
    case CS_SERVICE_NOTIFICATION_LCS_CLIENT_IDENTITY_IEI:
      if ((decoded_result = decode_lcs_client_identity (&cs_service_notification->lcsclientidentity, CS_SERVICE_NOTIFICATION_LCS_CLIENT_IDENTITY_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
...
int
encode_cs_service_notification (
  cs_service_notification_msg * cs_service_notification,
  uint8_t * buffer,
  uint32_t len)
{
  ...
  if ((encode_result = encode_paging_identity (&cs_service_notification->pagingidentity, 0, buffer + encoded, len - encoded)) < 0)      //Return in case of error
  ...
  if ((cs_service_notification->presencemask & CS_SERVICE_NOTIFICATION_SS_CODE_PRESENT)
    ...
    if ((encode_result = encode_ss_code (&cs_service_notification->sscode, CS_SERVICE_NOTIFICATION_SS_CODE_IEI, buffer + encoded, len - encoded)) < 0)
      // Return in case of error
      return encode_result;
    else
      encoded += encode_result;
  ...
  if ((cs_service_notification->presencemask & CS_SERVICE_NOTIFICATION_LCS_INDICATOR_PRESENT)
    ...
    if ((encode_result = encode_lcs_indicator (&cs_service_notification->lcsindicator, CS_SERVICE_NOTIFICATION_LCS_INDICATOR_IEI, buffer + encoded, len - encoded)) < 0)
      // Return in case of error
      return encode_result;
    else
      encoded += encode_result;
