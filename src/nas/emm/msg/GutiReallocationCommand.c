  ...
  uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case GUTI_REALLOCATION_COMMAND_TAI_LIST_IEI:
      if ((decoded_result = decode_tracking_area_identity_list (&guti_reallocation_command->tailist, GUTI_REALLOCATION_COMMAND_TAI_LIST_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
...
int
encode_guti_reallocation_command (
  guti_reallocation_command_msg * guti_reallocation_command,
  uint8_t * buffer,
  uint32_t len)
{
  ...
  if ((encode_result = encode_eps_mobile_identity (&guti_reallocation_command->guti, 0, buffer + encoded, len - encoded)) < 0)  //Return in case of error
  ...
  if ((guti_reallocation_command->presencemask & GUTI_REALLOCATION_COMMAND_TAI_LIST_PRESENT)
    ...
    if ((encode_result = encode_tracking_area_identity_list (&guti_reallocation_command->tailist, GUTI_REALLOCATION_COMMAND_TAI_LIST_IEI, buffer + encoded, len - encoded)) < 0)
