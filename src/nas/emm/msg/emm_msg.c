  ...
  uint32_t len)
{
  ...
  if (decode_result < 0) {
     ...
     */
    //TBD : There is some issue related to memory free for the logged message. Disabling this code till we resolve this issue
  ...
  uint32_t len)
{
 ...
 // bool                                    is_down_link = true;
  ...
  case NW_DETACH_REQUEST:
    encode_result = encode_nw_detach_request (&msg->nw_detach_request, buffer, len);
    break;
