...
//------------------------------------------------------------------------------
static int _emm_cn_nw_initiated_detach_ue (const uint32_t ue_id)
{
  int                                     rc = RETURNok;

  OAILOG_FUNC_IN (LOG_NAS_EMM);
  OAILOG_DEBUG (LOG_NAS_EMM, "EMM-PROC NW Initiated Detach UE" MME_UE_S1AP_ID_FMT "\n", ue_id);
...
static int _emm_cn_pdn_connectivity_fail (const emm_cn_pdn_fail_t * msg)
{
  ...
  switch (msg->cause) {
    ...
    case CAUSE_NO_RESOURCES_AVAILABLE:
      ...
      increment_counter ("ue_pdn_connectivity_req", 1, 2, "result", "failure", "cause", "no_resources_available");
      break;
    case CAUSE_ALL_DYNAMIC_ADDRESSES_OCCUPIED:
      esm_cause = ESM_CAUSE_INSUFFICIENT_RESOURCES;
      increment_counter ("ue_pdn_connectivity_req", 1, 2, "result", "failure", "cause", "all_dynamic_resources_occupied");
      break;
    default:
      esm_cause = ESM_CAUSE_REQUEST_REJECTED_BY_GW;
      break;
  ...
  if (size > 0) {
    ...
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "pdn_connection_estb_failed");
...
int emm_cn_send (const emm_cn_t * msg)
{
  ...
  case EMMCN_NW_INITIATED_DETACH_UE:
    rc = _emm_cn_nw_initiated_detach_ue (msg->u.emm_cn_nw_initiated_detach.ue_id);
    break;
