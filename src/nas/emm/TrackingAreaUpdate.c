  ...
  const nas_message_decode_status_t  * decode_status)
{
  ...
  *emm_cause = EMM_CAUSE_SUCCESS;
  ...
  if (! ue_ctx) {
    if (EPS_MOBILE_IDENTITY_GUTI == msg->oldguti.guti.typeofidentity) {
      ...
      if ( ue_ctx) {
        ...
        increment_counter ("tracking_area_update_req", 1, 2, "result", "failure", "cause", "ue_identify_cannot_be_derived");
  ...
  OAILOG_INFO (LOG_NAS_EMM, "EMM-PROC-  Tracking Area Update request. TAU_Type=%d, active_flag=%d)\n",
  ...
  if (EPS_UPDATE_TYPE_PERIODIC_UPDATING != msg->epsupdatetype.epsupdatetypevalue) {
    ...
    OAILOG_WARNING (LOG_NAS_EMM, "EMM-PROC- Sending Tracking Area Update Reject as TAU type = Normal.ue_id=" MME_UE_S1AP_ID_FMT ", cause=%d)\n",
        ue_id, EMM_CAUSE_IE_NOT_IMPLEMENTED);
    rc = emm_proc_tracking_area_update_reject (ue_id, EMM_CAUSE_IE_NOT_IMPLEMENTED);
    increment_counter ("tracking_area_update_req", 1, 2, "result", "failure", "cause", "normal_tau_not_supported");
    OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
  }
  #if 0
  /*
  ...
  if (msg->presencemask & TRACKING_AREA_UPDATE_REQUEST_EPS_BEARER_CONTEXT_STATUS_IEI) {
    ...
    OAILOG_WARNING (LOG_NAS_EMM, "EMM-PROC- Sending Tracking Area Update Reject.EPS Bearer Context Status IE not supported. ue_id=" MME_UE_S1AP_ID_FMT ", cause=%d)\n",
        ue_id, EMM_CAUSE_IE_NOT_IMPLEMENTED);
    rc = emm_proc_tracking_area_update_reject (ue_id, EMM_CAUSE_IE_NOT_IMPLEMENTED);
    increment_counter ("tracking_area_update_req", 1, 2, "result", "failure", "cause", "eps_bearer_context_status_ie_not_supported");
    OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
  ...
  if (emm_ctx) {
   ...
   if (data->active_flag) {
      ...
      if (rc != RETURNerror) {
        ...
        increment_counter ("tracking_area_update", 1, 1, "action", " initial_ictr_tau_accept_sent");
      }
    } // Active Flag
    else {
      ...
      rc = emm_sap_send (&emm_sap);
      increment_counter ("tracking_area_update", 1, 1, "action", "tau_accept_sent");
    }
  } else {
