...
)
{
  ...
  enc_nw_name = bfromcstr((const char*)result);
  emm_as->full_network_name = bstrcpy((const_bstring) enc_nw_name);
  bdestroy(enc_nw_name);
  /*
  ...
  enc_nw_name = bfromcstr((const char*)result);
  emm_as->short_network_name = bstrcpy((const_bstring) enc_nw_name);
  bdestroy(enc_nw_name);
...
static void _emm_information_pack_gsm_7Bit(bstring  str, unsigned char *result)
{
   ...
   while (i <= len)
   {
      if (i < len)
      {
         result[encIdx++] = ((bchar(str,i) >> j)| ((bchar(str,i + 1) << (7 - j)) & 0xFF));
      }
      else
      {
