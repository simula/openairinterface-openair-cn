...
#define           EMM_CTXT_MEMBER_CLEAR_BIT( eMmCtXtMemBeRmAsK, bIt ) do { (eMmCtXtMemBeRmAsK) &= ~bIt;} while (0)
  ...
  plmn_t                   serving_network_id; /* PLMN id in the format specfied in 33.401.
                                                 It is passed to HSS in S6a:Auth Info Request message.
                                                 HSS uses it to calculate Kasme. */
...
#define EMM_AUTHENTICATION_SYNC_FAILURE_MAX   2
  ...
  struct nas_timer_t       T3422; /* Detach timer         */
  void *t3422_arg;
