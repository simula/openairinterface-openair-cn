  ...
  uint32_t * const tmsi)
{
  ...
  if (emm_ctx) {
    ...
    if (ident_proc){
       ...
       */
      void *unused = NULL;
      nas_stop_T3470(ue_id, &ident_proc->T3470, unused);
      ...
      if (imsi) {
        ...
        if(imsi_emm_ctx_duplicate){ /**< We have the UE with this IMSI (different GUTI). */
          if(imsi_emm_ctx_duplicate->ue_id != emm_ctx->ue_id){
			  ...
			  void *unused = NULL;
			  nas_stop_T_retry_specific_procedure(ue_id, &((nas_emm_specific_proc_t*)(((nas_emm_base_proc_t *)ident_proc)->parent))->retry_timer, unused);
			  nas_start_T_retry_specific_procedure(ue_id, &((nas_emm_specific_proc_t*)(((nas_emm_base_proc_t *)ident_proc)->parent))->retry_timer, ((nas_emm_specific_proc_t*)(((nas_emm_base_proc_t *)ident_proc)->parent))->retry_cb);
...
static void _identification_t3470_handler (void *args)
{
  ...
  if (!(emm_ctx)) {
    OAILOG_ERROR (LOG_NAS_EMM, "T3470 timer expired No EMM context for UE "MME_UE_S1AP_ID_FMT ". \n", ue_id);
   ...
   /*
    ...
    if (ident_proc->retransmission_count < IDENTIFICATION_COUNTER_MAX) {
      ...
      */
       ...
       */
      if((emm_ctx = emm_data_context_get(&_emm_data, ue_id))){
    	  OAILOG_WARNING (LOG_NAS_EMM, "EMM-PROC  - EMM Context for ueId " MME_UE_S1AP_ID_FMT " is still existing after aborted identification. "
    			  "Removing failed EMM context.. \n", ue_id);
...
static int _identification_request (nas_emm_ident_proc_t * const proc)
{
  ...
  if (rc != RETURNerror) {
     ...
     * Start T3470 timer
     */
    nas_start_T3470(proc->ue_id, &proc->T3470, proc->emm_com_proc.emm_proc.base_proc.time_out, (void*)proc->ue_id);
  }
