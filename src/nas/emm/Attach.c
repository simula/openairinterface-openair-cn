  ...
  const nas_message_decode_status_t  * const decode_status)
{
   ...
   */
   ...
   if (new_emm_ctx == NULL)
   {
    ...
    if (imsi) {
      ...
      if (imsi_emm_ctx) {
        ...
        if (emm_ctx_is_common_procedure_running(imsi_emm_ctx, EMM_CTXT_COMMON_PROC_SMC)) {
          ...
          // Allocate new context and process the new request as fresh attach request
          create_new_emm_ctxt = true;
          increment_counter ("duplicate_attach_request", 1, 1, "action", "processed_old_ctxt_cleanup");
        }
        if (emm_ctx_is_common_procedure_running(imsi_emm_ctx, EMM_CTXT_COMMON_PROC_IDENT)) {
          if (emm_ctx_is_specific_procedure(imsi_emm_ctx, EMM_CTXT_SPEC_PROC_ATTACH)) {
            if (emm_ctx_is_specific_procedure(imsi_emm_ctx, EMM_CTXT_SPEC_PROC_ATTACH_ACCEPT_SENT | EMM_CTXT_SPEC_PROC_ATTACH_REJECT_SENT)) {
              ...
              increment_counter ("duplicate_attach_request", 1, 1, "action", "not_handled");
            } else {
        ...
        if (EMM_REGISTERED == fsm_state) {
          ...
          if (imsi_emm_ctx->is_has_been_attached) {
            OAILOG_DEBUG (LOG_NAS_EMM, "EMM-PROC  - the new ATTACH REQUEST is progressed\n");
            // Trigger clean up
            emm_sap_t                               emm_sap = {0};
            emm_sap.primitive = EMMCN_IMPLICIT_DETACH_UE;
          ...
          if (_emm_attach_have_changed (imsi_emm_ctx, type, ksi, guti, imsi, imei, eea, eia, ucs2, uea, uia, gea, umts_present, gprs_present)) {
             ...
             */
              ...
              increment_counter ("duplicate_attach_request", 1, 1, "action", "ignored_duplicate_req_retx_attach_accept");
          ...
          if (_emm_attach_have_changed (imsi_emm_ctx, type, ksi, guti, imsi, imei, eea, eia, ucs2, uea, uia, gea, umts_present, gprs_present)) {
             ...
             */
             ...
             increment_counter ("duplicate_attach_request", 1, 1, "action", "ignored");
             OAILOG_FUNC_RETURN (LOG_NAS_EMM, RETURNok);
          }
        }
    ...
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "existing_guti");
    OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
    #if 0
  ...
  if (create_new_emm_ctxt) {
    ...
    if (!new_emm_ctx) {
       ...
       */
      rc = _emm_attach_reject (&ue_ctx);
      increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "memory_allocation_failure");
      OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
    }
    ...
    new_emm_ctx->T3422.id = NAS_TIMER_INACTIVE_ID;
    new_emm_ctx->T3422.sec = T3422_DEFAULT_VALUE;
    ...
    if (rc != RETURNok) {
      ...
      rc = _emm_attach_reject (new_emm_ctx);
      increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "emm_context_updation_failure");
    } else {
      /*
  ...
  const_bstring esm_msg_pP)
{
  ...
  if ((rc != RETURNerror) && (esm_sap.err == ESM_SAP_SUCCESS)) {
    ...
    if (rc == RETURNok) {
       ...
       * */
      emm_proc_emm_informtion( (emm_data_context_t *) emm_ctx);
  ...
  void *args)
{
  ...
  if (IS_EMM_CTXT_PRESENT_IMSI(emm_ctx)) {
    ...
    if (!IS_EMM_CTXT_PRESENT_AUTH_VECTORS(emm_ctx)) {
      ...
      nas_itti_auth_info_req (emm_ctx->ue_id, emm_ctx->_imsi64, true, &emm_ctx->serving_network_id,
                              MAX_EPS_AUTH_VECTORS, NULL,emm_ctx->_imsi.length);
      rc = RETURNok;
    } else {
      ksi_t                                   eksi = 0;
      int                                     vindex = 0;
      ...
      /*if (!IS_EMM_CTXT_PRESENT_AUTH_VECTORS(emm_ctx)) {
        // Ask upper layer to fetch new security context
        nas_itti_auth_info_req (emm_ctx->ue_id, emm_ctx->_imsi64, true, &emm_ctx->serving_network_id, MAX_EPS_AUTH_VECTORS, NULL);
        rc = RETURNok;
      } else */{
    ...
    if (rc != RETURNok) {
      ...
      // Do not accept the UE to attach to the network
      increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "internal_software_error");
     ...
     */
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "emergency_attach");
    OAILOG_ERROR (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT "EMM-PROC  - IMEI based attach is not supported\n", emm_ctx->ue_id);
    if (rc != RETURNok) {
  ...
  if (rc != RETURNok) {
    // Do not accept the UE to attach to the network
    rc = _emm_attach_reject (emm_ctx);
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "attach_prodecure_error");
  }
  ...
  void *args)
{
  ...
  if ((rc != RETURNerror) && (esm_sap.err == ESM_SAP_SUCCESS)) {
    ...
    rc = _emm_attach_reject (emm_ctx);
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "esm_failure");
  } else {
    /*
     * ESM procedure failed and, received message has been discarded or
     * Status message has been returned; ignore ESM procedure failure
  ...
  if (rc != RETURNok) {
     ...
     */
    rc = _emm_attach_reject (emm_ctx);
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "protocol_error");
  }
  ...
  const_bstring esm_msg_pP)
{
  ...
  /*
   * Update serving_network_id to be passed to HSS in order to get
   * authentication vectors.
   * HSS uses serving_network_id to calculate Kasme
   */
  ctx->serving_network_id.mcc_digit2 = ctx->originating_tai.plmn.mcc_digit2;
  ctx->serving_network_id.mcc_digit1 = ctx->originating_tai.plmn.mcc_digit1;
  ctx->serving_network_id.mnc_digit3 = ctx->originating_tai.plmn.mnc_digit3;
  ctx->serving_network_id.mcc_digit3 = ctx->originating_tai.plmn.mcc_digit3;

  /*
   * check whether MNC length is 3 digit or 2 digit.
   */

  if (ctx->originating_tai.plmn.mnc_digit3 == 0xf) {
    /*
     * This implies MNC length is 2 digit. And encoding of serving network id
     * is same as encoding of PLMN id received in S1 message from eNB.
     */
    ctx->serving_network_id.mnc_digit2 = ctx->originating_tai.plmn.mnc_digit2;
    ctx->serving_network_id.mnc_digit1 = ctx->originating_tai.plmn.mnc_digit1;
    OAILOG_DEBUG (LOG_NAS_EMM, "serving_plmn mnc len = 2 digits \n");
  } else {
    /*
     * This implies MNC length is 3 digit. And encoding of serving network id
     * is different compared to encoding of PLMN id received in S1 message from eNB.
     */
    ctx->serving_network_id.mnc_digit2 = ctx->originating_tai.plmn.mnc_digit1;
    ctx->serving_network_id.mnc_digit1 = ctx->originating_tai.plmn.mnc_digit2;
    OAILOG_DEBUG (LOG_NAS_EMM, "serving_plmn mnc len = 3 digits \n");
  }
