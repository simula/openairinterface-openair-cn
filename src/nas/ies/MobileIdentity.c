  ...
  uint32_t len)
{
  ...
  if (typeofidentity != MOBILE_IDENTITY_NOT_AVAILABLE) {
    ...
    if (typeofidentity == MOBILE_IDENTITY_IMSI) {
      ...
      decoded_rc = decode_imei_mobile_identity (&mobileidentity->imei, buffer + decoded);
    } else if (typeofidentity == MOBILE_IDENTITY_IMEISV) {
      decoded_rc = decode_imeisv_mobile_identity (&mobileidentity->imeisv, buffer + decoded);
    } else if (typeofidentity == MOBILE_IDENTITY_TMSI) {
      decoded_rc = decode_tmsi_mobile_identity (&mobileidentity->tmsi, buffer + decoded);
    } else if (typeofidentity == MOBILE_IDENTITY_TMGI) {
      decoded_rc = decode_tmgi_mobile_identity (&mobileidentity->tmgi, buffer + decoded);
    }
  } else if (ielen == MOBILE_IDENTITY_NOT_AVAILABLE_LTE_LENGTH) {
    decoded_rc = decode_no_mobile_identity (&mobileidentity->no_id, buffer + decoded);
  }
  ...
  uint32_t len)
{
  ...
  if (mobileidentity->no_id.typeofidentity != MOBILE_IDENTITY_NOT_AVAILABLE) {
    if (mobileidentity->imsi.typeofidentity == MOBILE_IDENTITY_IMSI) {
      encoded_rc = encode_imsi_mobile_identity (&mobileidentity->imsi, buffer + encoded);
    } else if (mobileidentity->imei.typeofidentity == MOBILE_IDENTITY_IMEI) {
      encoded_rc = encode_imei_mobile_identity (&mobileidentity->imei, buffer + encoded);
    } else if (mobileidentity->imeisv.typeofidentity == MOBILE_IDENTITY_IMEISV) {
      encoded_rc = encode_imeisv_mobile_identity (&mobileidentity->imeisv, buffer + encoded);
    } else if (mobileidentity->tmsi.typeofidentity == MOBILE_IDENTITY_TMSI) {
      encoded_rc = encode_tmsi_mobile_identity (&mobileidentity->tmsi, buffer + encoded);
    } else if (mobileidentity->tmgi.typeofidentity == MOBILE_IDENTITY_TMGI) {
      encoded_rc = encode_tmgi_mobile_identity (&mobileidentity->tmgi, buffer + encoded);
    }
    ...
    encoded_rc = encode_no_mobile_identity (&mobileidentity->no_id, buffer + encoded);
  ...
  uint8_t * buffer)
{
  ...
  ielen = *(buffer + decoded) ; /* Pointing buffer to IE length field, to include the ieLen byte*/
  decoded++;
...
static int
encode_tmgi_mobile_identity (
  TmgiMobileIdentity_t * tmgi,
  uint8_t * buffer)
{
  uint32_t                                encoded = 0;

  *(buffer + encoded) = 0x00 | ((tmgi->mbmssessionidindication & 0x1) << 5) | ((tmgi->mccmncindication & 0x1) << 4) | ((tmgi->oddeven & 0x1) << 3) | (tmgi->typeofidentity & 0x7);
  encoded++;
  IES_ENCODE_U24 (buffer, encoded, tmgi->mbmsserviceid);
  *(buffer + encoded) = 0x00 | ((tmgi->mccdigit2 & 0xf) << 4) | (tmgi->mccdigit1 & 0xf);
  encoded++;
  *(buffer + encoded) = 0x00 | ((tmgi->mncdigit3 & 0xf) << 4) | (tmgi->mccdigit3 & 0xf);
  encoded++;
  *(buffer + encoded) = 0x00 | ((tmgi->mncdigit2 & 0xf) << 4) | (tmgi->mncdigit1 & 0xf);
  encoded++;
