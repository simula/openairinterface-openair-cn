  ...
  uint32_t len)
{
  ...
  encoded++;
  for(indx = 0; indx < SUPPORTED_CODEC_LIST_NUMBER_OF_SYSTEM_INDICATION; indx++) {
     ...
     *(buffer + encoded) = supportedcodeclist[indx]->lengthofbitmap;
     ...
     if(supportedcodeclist[indx]->lengthofbitmap == sizeof(uint8_t)) {
       ...
       OAILOG_ERROR(LOG_NAS,"Encode supported codec list: Invalid Bitmal length: %d \n",
       supportedcodeclist[indx]->lengthofbitmap);
     }
  }
