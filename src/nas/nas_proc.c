    ...
    s6a_auth_info_ans_t * aia)
{
  ...
  if ((aia->result.present == S6A_RESULT_BASE)
     ...
     */
    if (aia->result.present == S6A_RESULT_BASE) {
      OAILOG_ERROR (LOG_NAS_EMM, "Auth info Rsp failure for imsi " IMSI_64_FMT ", base_error_code %d \n", imsi64,
                                                                                            aia->result.choice.base);
      rc = nas_proc_auth_param_fail (ctxt->ue_id, s6a_error_2_nas_cause (aia->result.choice.base, 0));
    } else {
      OAILOG_ERROR (LOG_NAS_EMM, "Auth info Rsp failure for imsi " IMSI_64_FMT ", experimental_error_code %d \n", imsi64,
