...
static int
_nas_timer_db_get_id (
  void);

static int
_nas_timer_db_is_active (
  int id);

static nas_timer_entry_t *
_nas_timer_db_create_entry (
  long sec,
  nas_timer_callback_t cb,
  void *args);

static void
_nas_timer_db_delete_entry (
  int id);
...
static nas_timer_entry_t *
_nas_timer_db_remove_entry (
  int id);

static int
_nas_timer_db_remove (
  timer_queue_t * entry);
...
#if ENABLE_ITTI
  // nas_timer doesn't use the args given to timer_setup, so passing in NULL
  ret = timer_setup (sec, 0, TASK_NAS_MME, INSTANCE_DEFAULT, TIMER_ONE_SHOT, NULL, 0, &timer_id);
  ...
  int id)
{
  ...
  if (_nas_timer_db_is_active (id)) {
    ...
    _nas_timer_db_insert_entry (id, te);
  #if ENABLE_ITTI
    ret = timer_setup (te->itv.tv_sec, 0, TASK_NAS_MME, INSTANCE_DEFAULT, TIMER_ONE_SHOT, NULL, 0, &(te->timer_id));
...
_nas_timer_db_is_active (
  int id)
{
  return ((id != NAS_TIMER_INACTIVE_ID) && (_nas_timer_db.tq[id].id != NAS_TIMER_INACTIVE_ID));
}
