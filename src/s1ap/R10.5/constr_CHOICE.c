...
CHOICE_compare(asn_TYPE_descriptor_t *td1, void *sptr1, asn_TYPE_descriptor_t *td2, void *sptr2)
{
  ...
  if(present1 > 0 && present1 <= td1->elements_count) {
    ...
    if((elm1->flags & ATF_POINTER) && (elm2->flags & ATF_POINTER)){
      memb_ptr1 = *(void * const *)((char *)sptr1 + elm1->memb_offset);
      memb_ptr2 = *(void * const *)((char *)sptr2 + elm2->memb_offset);
    ...
    } else if (!(elm1->flags & ATF_POINTER) && !(elm2->flags & ATF_POINTER)){
      memb_ptr1 = (void *)((char *)sptr1 + elm1->memb_offset);
      memb_ptr2 = (void *)((char *)sptr2 + elm2->memb_offset);
    } else {
      res = calloc(1, sizeof(asn_comp_rval_t));
      res->name = td1->name;
      res->structure1 = sptr1;
