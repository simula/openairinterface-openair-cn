  ...
  const itti_mme_app_connection_establishment_cnf_t * const conn_est_cnf_pP)
{
  ...
  S1ap_E_RABToBeSetupItemCtxtSUReq_t     *e_RABToBeSetup = NULL;
  ...
  e_RABToBeSetup = calloc(1, sizeof *e_RABToBeSetup);
  ...
  if (conn_est_cnf_pP->nas_conn_est_cnf.nas_msg != NULL) {
    ...
    if(nas_pdu->size) {
      nas_pdu->buf  = calloc(1, nas_pdu->size);
      memcpy(nas_pdu->buf, conn_est_cnf_pP->nas_conn_est_cnf.nas_msg->data,
             nas_pdu->size);
    }
...
#  endif
  ...
  initialContextSetupRequest_p->ueSecurityCapabilities.encryptionAlgorithms.buf = calloc (2, sizeof(uint8_t));
  memcpy(initialContextSetupRequest_p->ueSecurityCapabilities.encryptionAlgorithms.buf,
  ...
  initialContextSetupRequest_p->ueSecurityCapabilities.integrityProtectionAlgorithms.buf = calloc (2, sizeof(uint8_t));
  memcpy(initialContextSetupRequest_p->ueSecurityCapabilities.integrityProtectionAlgorithms.buf,
