...
int encode_tmsi_status(tmsi_status_t *tmsistatus, const bool iei_present, uint8_t *buffer, const uint32_t len)
{
  ...
  *(buffer + encoded) = 0x00 | (GMM_TMSI_STATUS_IEI & 0xf0) | (*tmsistatus & 0x1);
  encoded++;
  return encoded;
}
  ...
  const uint32_t len)
{
  ...
  if (iei_present) {
    CHECK_PDU_POINTER_AND_LENGTH_DECODER (buffer, P_TMSI_SIGNATURE_IE_MAX_LENGTH, len);
  ...
  const uint32_t len)
{
  ...
  if (iei_present) {
    CHECK_PDU_POINTER_AND_LENGTH_ENCODER (buffer, P_TMSI_SIGNATURE_IE_MAX_LENGTH, len);
  ...
  const uint32_t len)
{
  ...
  CHECK_PDU_POINTER_AND_LENGTH_DECODER (buffer, IMEISV_REQUEST_IE_MAX_LENGTH, len);
  ...
  const uint32_t len)
{
   ...
   * Checking length and pointer
   */
  CHECK_PDU_POINTER_AND_LENGTH_ENCODER (buffer, IMEISV_REQUEST_IE_MAX_LENGTH, len);
  *(buffer + encoded) = 0x00;
  if (is_ie_present) {
  ...
  const uint32_t len)
{
  ...
  if (iei_present) {
    CHECK_PDU_POINTER_AND_LENGTH_DECODER (buffer, MS_NETWORK_CAPABILITY_IE_MIN_LENGTH, len);
    CHECK_IEI_DECODER (GMM_MS_NETWORK_CAPABILITY_IEI, *buffer);
    decoded++;
  } else {
    CHECK_PDU_POINTER_AND_LENGTH_DECODER (buffer, (MS_NETWORK_CAPABILITY_IE_MIN_LENGTH - 1), len);
  }
    ...
    if (ielen > 2) {
      ...
      if (ielen > 3) {
        ...
        msnetworkcapability->gia4 =
            (b & MS_NETWORK_CAPABILITY_GIA4) >> 6;
        msnetworkcapability->gia5 =
            (b & MS_NETWORK_CAPABILITY_GIA5) >> 5;
        ...
        msnetworkcapability->rest_use_enhanc_cov_cap =
            (b & MS_NETWORK_CAPABILITY_RESTRICTION_ON_USE_OF_ENHANCED_COVERAGE_CAPABILITY)
            >> 1;
  ...
  const uint32_t len)
{
   ...
   */
  CHECK_PDU_POINTER_AND_LENGTH_ENCODER (buffer, MS_NETWORK_CAPABILITY_IE_MIN_LENGTH, len);
    ...
    const uint32_t len)
{
  ...
  if (iei_present) {
    CHECK_IEI_DECODER (GMM_VOICE_DOMAIN_PREFERENCE_AND_UE_USAGE_SETTING_IEI, *buffer);
    decoded++;
  }

  memset (voicedomainpreferenceandueusagesetting, 0, sizeof (voice_domain_preference_and_ue_usage_setting_t));
