  ...
  struct ue_context_s *const ue_context_pP)
{
  ...
  IMSI64_TO_STRING (imsi, s6a_ulr_p->imsi, ue_context_pP->pending_pdn_connectivity_req_imsi_length);
  ...
  const s6a_update_location_ans_t * const ula_pP)
{
  ...
  ue_context_p->subscribed_ambr.br_ul = ue_context_p->subscribed_ambr.br_ul;
  ue_context_p->subscribed_ambr.br_dl = ue_context_p->subscribed_ambr.br_dl;
  ...
  const s6a_cancel_location_req_t * const clr_pP)
{
  ...
  OAILOG_DEBUG (LOG_MME_APP, "%s S6a Cancel Location Request for imsi " IMSI_64_FMT "\n", __FUNCTION__, imsi);
  ...
  if (clr_pP->cancellation_type != SUBSCRIPTION_WITHDRAWL) {
    OAILOG_ERROR (LOG_MME_APP, "S6a Cancel Location Request: Cancellation_type not supported %d \n", clr_pP->cancellation_type);
    OAILOG_FUNC_RETURN (LOG_MME_APP, RETURNerror);
  }
  /*
  ...
  if (ue_context_p->ecm_state == ECM_IDLE) {
    ...
    message_p = itti_alloc_new_message (TASK_MME_APP, NAS_NW_INITIATED_DETACH_UE_REQ);
    DevAssert (message_p);
    message_p->ittiMsg.nas_nw_initiated_detach_ue_req.ue_id = ue_context_p->mme_ue_s1ap_id;
    itti_send_msg_to_task (TASK_NAS_MME, INSTANCE_DEFAULT, message_p);
  }
  OAILOG_FUNC_RETURN (LOG_MME_APP, rc);
}
