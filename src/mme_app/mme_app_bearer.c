...
static int mme_app_paging_request_helper(ue_context_t *ue_context_p,
                                         bool set_timer);
  ...
  struct ue_context_s *const ue_context_pP)
{
  ...
  memcpy((char *)session_request_p->imsi.digit, ue_context_pP->pending_pdn_connectivity_req_imsi,
                                          ue_context_pP->pending_pdn_connectivity_req_imsi_length);
...
//#pragma message  "Check ue_context_p ambr"
  ...
  if (timer_setup (ue_context_p->initial_context_setup_rsp_timer.sec, 0,
                TASK_MME_APP, INSTANCE_DEFAULT, TIMER_ONE_SHOT, (void *) &(ue_context_p->mme_ue_s1ap_id), sizeof(mme_ue_s1ap_id_t), &(ue_context_p->initial_context_setup_rsp_timer.id)) < 0) {
  ...
  itti_mme_app_initial_ue_message_t * const initial_pP)
{
      ...
      if (ue_nas_ctx)
      {
        ...
        if ((ue_context_p != NULL) && (ue_context_p->mme_ue_s1ap_id == ue_nas_ctx->ue_id)) {
          ...
          if (ue_context_p->paging_response_timer.id
              ...
              if (timer_remove (ue_context_p->paging_response_timer.id)) {
  ...
  ue_context_p->paging_response_timer.sec = MME_APP_PAGING_RESPONSE_TIMER_VALUE;
...
mme_app_handle_mobile_reachability_timer_expiry (struct ue_context_s *ue_context_p)
{
  ...
  if (timer_setup (ue_context_p->implicit_detach_timer.sec, 0,
                TASK_MME_APP, INSTANCE_DEFAULT, TIMER_ONE_SHOT, (void *)&(ue_context_p->mme_ue_s1ap_id), sizeof(mme_ue_s1ap_id_t), &(ue_context_p->implicit_detach_timer.id)) < 0) {
...
mme_app_handle_initial_context_setup_rsp_timer_expiry (struct ue_context_s *ue_context_p)
{
  ...
  if (ue_context_p->mm_state == UE_UNREGISTERED) {
    ...
    message_p->ittiMsg.nas_implicit_detach_ue_ind.ue_id = ue_context_p->mme_ue_s1ap_id;
    itti_send_msg_to_task (TASK_NAS_MME, INSTANCE_DEFAULT, message_p);
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "no_context_setup_rsp_from_enb");
    ...
    increment_counter ("service_request", 1, 2, "result", "failure", "cause", "no_context_setup_rsp_from_enb");
  }
  OAILOG_FUNC_OUT (LOG_MME_APP);
}
//------------------------------------------------------------------------------
  ...
  const itti_mme_app_initial_context_setup_failure_t * const initial_ctxt_setup_failure_pP)
{
  ...
  if (ue_context_p->mm_state == UE_UNREGISTERED) {
    ...
    message_p->ittiMsg.nas_implicit_detach_ue_ind.ue_id = ue_context_p->mme_ue_s1ap_id;
    itti_send_msg_to_task (TASK_NAS_MME, INSTANCE_DEFAULT, message_p);
    increment_counter ("ue_attach", 1, 2, "result", "failure", "cause", "initial_context_setup_failure_rcvd");
    increment_counter ("ue_attach", 1, 1, "action", "attach_abort");
  } else {
    // Release S1-U bearer and move the UE to idle mode
    mme_app_send_s11_release_access_bearers_req(ue_context_p);
    increment_counter ("service_request", 1, 2, "result", "failure", "cause", "initial_context_setup_failure_rcvd");
  }
  OAILOG_FUNC_OUT (LOG_MME_APP);
}
//------------------------------------------------------------------------------
 ...
 *
 * @param ue_imsi - uint64_t value of UE IMSI
...
static int mme_app_paging_request_helper(ue_context_t *ue_context_p,
  ...
  paging_request->imsi = ue_context_p->imsi;
  ...
  int timer_rc = timer_setup (ue_context_p->paging_response_timer.sec, 0,
                   TASK_MME_APP, INSTANCE_DEFAULT, TIMER_ONE_SHOT,
                   (void *) &(ue_context_p->mme_ue_s1ap_id),
                   sizeof(mme_ue_s1ap_id_t),
                   &(ue_context_p->paging_response_timer.id));
  if (timer_rc < 0) {
    OAILOG_ERROR (LOG_MME_APP, "Failed to start paging timer for ue %d\n",
                  ue_context_p->mme_ue_s1ap_id);
  }
...
int mme_app_handle_initial_paging_request (const char* imsi) {
  ...
  ue_context_p = mme_ue_context_exists_imsi(&mme_app_desc.mme_ue_contexts,
                                            imsi64);
  if (ue_context_p == NULL) {
    OAILOG_ERROR(LOG_MME_APP, "Unknown IMSI%s, could not initiate paging\n",
                 imsi);
    mme_ue_context_dump_coll_keys();
    OAILOG_FUNC_RETURN (LOG_MME_APP, RETURNerror);
  }
  return mme_app_paging_request_helper(ue_context_p, true);
}


int mme_app_handle_paging_timer_expiry (ue_context_t *ue_context_p) {
  ue_context_p->paging_response_timer.id = MME_APP_TIMER_INACTIVE_ID;
  return mme_app_paging_request_helper(ue_context_p, false);
}
