...
//  }
  ...
  if(((struct sockaddr*)&nas_pdn_disconnect_req_pP->saegw_s11_ip_addr)->sa_family != 0){
    ...
    rc =  mme_app_send_delete_session_request(ue_context, nas_pdn_disconnect_req_pP->default_ebi, (const struct sockaddr*)&nas_pdn_disconnect_req_pP->saegw_s11_ip_addr,
...
//        if ((BEARER_STATE_SGW_CREATED  || BEARER_STATE_S1_RELEASED) & bc_session->bearer_state) {    /**< It could be in IDLE mode. */
          ...
          /** For GBR Bearers add the BR values. */
          establishment_cnf_p->gbr_qos_information[establishment_cnf_p->no_of_e_rabs].e_rab_guaranteed_bit_rate_downlink = bc_context->bearer_level_qos.gbr.br_dl;
          establishment_cnf_p->gbr_qos_information[establishment_cnf_p->no_of_e_rabs].e_rab_guaranteed_bit_rate_uplink = bc_context->bearer_level_qos.gbr.br_ul;
          establishment_cnf_p->gbr_qos_information[establishment_cnf_p->no_of_e_rabs].e_rab_maximum_bit_rate_downlink = bc_context->bearer_level_qos.mbr.br_dl;
          establishment_cnf_p->gbr_qos_information[establishment_cnf_p->no_of_e_rabs].e_rab_maximum_bit_rate_uplink = bc_context->bearer_level_qos.mbr.br_ul;
			    ...
			    if(default_bc->bearer_state & BEARER_STATE_ACTIVE) {
			      if(!s11_proc->num_bearers_unhandled) {
			    	  mme_app_send_s11_create_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11,
			    			  ue_session_pool->privates.fields.saegw_teid_s11, 0, (void*)s11_proc_cbr->proc.s11_trxn, s11_proc_cbr->bcs_tbc);
				  ...
				  if(default_bc->bearer_state & BEARER_STATE_ACTIVE) {
				      if(!s11_proc->num_bearers_unhandled) {
				    	  mme_app_send_s11_update_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11,
								  ...
								  (void*)s11_proc_ubr->proc.s11_trxn, s11_proc_ubr->bcs_tbu);
				  ...
				  if(default_bearer->bearer_state & BEARER_STATE_ACTIVE) {
					  if(!s11_proc->num_bearers_unhandled) {
						  mme_app_send_s11_delete_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11,
								  ...
								  (void*)s11_proc_dbr->proc.s11_trxn, &s11_proc_dbr->ebis);
...
//	    	  for(int num_ebi = 0; num_ebi < s11_proc_cbr->bcs_tbc->num_bearer_context; num_ebi++) {
//	    		  if(s11_proc_cbr->bcs_tbc->bearer_contexts[num_ebi].eps_bearer_id == itti_nas_erab_setup_req->ebi){
//	    		    bc_tbc = &s11_proc_cbr->bcs_tbc->bearer_contexts[num_ebi];
//	    		    break;
//	    		  }
//	    	  }
	    	  // if(bc_tbc && bc_tbc->cause.cause_value == REQUEST_ACCEPTED){
	    	  if(s11_proc_cbr->proc.proc.in_progress){
	    		/** Only, if this is not a retry (ESM Timeout not treated as retry, only through MBR, TAU-Complete). */
	    		if(!itti_nas_erab_setup_req->retry) {
		    	  ...
		    	  NAS_DOWNLINK_DATA_REQ (message_p).nas_msg                = itti_nas_erab_setup_req->nas_msg;
		    	  itti_nas_erab_setup_req->nas_msg                   	   = NULL;
		    	  itti_send_msg_to_task (TASK_S1AP, INSTANCE_DEFAULT, message_p);
		    	  OAILOG_WARNING(LOG_MME_APP, "Bearer context for ebi %d has been established in the eNodeB already (state %d) for UE " MME_UE_S1AP_ID_FMT " and has a positive value. "
  		  ...
  		  OAILOG_WARNING(LOG_MME_APP, "No NAS message existent for bearer context activation for ebi %d or UE " MME_UE_S1AP_ID_FMT ". Continuing with bearer procedure. \n",
		  				  bearer_context->ebi, ue_session_pool->privates.mme_ue_s1ap_id);
	  }
	}
	...
	if(s11_proc_cbr) {
	  if(!s11_proc_cbr->proc.proc.in_progress){
		...
		s11_proc_cbr->proc.proc.in_progress_count = itti_nas_erab_setup_req->retx_count;
	  } else {
	    /** Check if it is a retry, if so don't continue. */
  		if(itti_nas_erab_setup_req->retry) {
  		  /** Check if it is the same retx count. */
  		  if(s11_proc_cbr->proc.proc.in_progress_count != itti_nas_erab_setup_req->retx_count){
  		    OAILOG_ERROR( LOG_MME_APP, "S11 Create Bearer process for UE " MME_UE_S1AP_ID_FMT " is in progress and we received a retry. Not continuing with it (prog_count=%d, retx_count=%d).\n", ue_session_pool->privates.mme_ue_s1ap_id,
  		    	s11_proc_cbr->proc.proc.in_progress_count, itti_nas_erab_setup_req->retx_count);
  		    OAILOG_FUNC_OUT (LOG_MME_APP);
  		  } else {
  			OAILOG_INFO( LOG_MME_APP, "S11 Create Bearer process for UE " MME_UE_S1AP_ID_FMT " is in progress and we received a retry. Continuing with it (prog_count=%d == retx_count=%d).\n", ue_session_pool->privates.mme_ue_s1ap_id,
  				s11_proc_cbr->proc.proc.in_progress_count, itti_nas_erab_setup_req->retx_count);
  		  }
  		}
	  }
    ...
    message_p = itti_alloc_new_message (TASK_MME_APP, S1AP_E_RAB_SETUP_REQ);
...
mme_app_handle_nas_erab_modify_req (itti_nas_erab_modify_req_t * const itti_nas_erab_modify_req)
{
  ...
  mme_app_s11_proc_update_bearer_t * s11_proc_ubr = mme_app_get_s11_procedure_update_bearer(ue_session_pool);
  DevAssert(s11_proc_ubr);
...
//					  bc_tbu = &s11_proc_ubr->bcs_tbu->bearer_contexts[num_ebi];
//					  break;
//					}
//				}
  ...
  const itti_s11_delete_session_response_t * const delete_sess_resp_pP)
{
  ...
  if (!ue_context) {
    ...
    OAILOG_INFO (LOG_MME_APP, "We didn't find this teid in list of UE: %08x\n", delete_sess_resp_pP->teid);
    OAILOG_FUNC_OUT (LOG_MME_APP);
  }
  ...
  itti_s11_create_session_response_t * const create_sess_resp_pP)
{
  ...
  if(!pdn_connections){
    ...
    mme_app_itti_nas_pdn_connectivity_response(mme_ue_s1ap_id, create_sess_resp_pP->bearer_contexts_created.bearer_context[0].eps_bearer_id, create_sess_resp_pP->cause.cause_value);
  ...
  if(s10_handover_procedure){
    ...
    mme_app_send_s10_forward_relocation_response_err(s10_handover_procedure->remote_mme_teid.teid,
        ...
        (void*)s10_handover_procedure->forward_relocation_trxn, RELOCATION_FAILURE);
  ...
  const itti_s11_modify_bearer_response_t * const modify_bearer_resp_pP)
{
  ...
  mme_app_get_session_bearer_context_from_all(ue_session_pool, modify_bearer_resp_pP->bearer_contexts_modified.bearer_context[0].eps_bearer_id, &current_bearer_p);
  ...
  if(!current_bearer_p){
    OAILOG_INFO(LOG_MME_APP, "We could not find the first bearer with eBI %d in the set of bearers. Ignoring the MBResp for UE with ueId: " MME_UE_S1AP_ID_FMT ". \n",
    		modify_bearer_resp_pP->bearer_contexts_modified.bearer_context[0].eps_bearer_id, ue_context->privates.mme_ue_s1ap_id);
    OAILOG_FUNC_RETURN (LOG_MME_APP, RETURNok);
  }
  ...
  if(modify_bearer_resp_pP->internal_flags & INTERNAL_FLAG_X2_HANDOVER) {
    ...
    bearer_contexts_to_be_created_t  bcs_tbs;
    ...
    if(s10_handover_procedure->failed_ebi_list.num_ebi){
      ...
      if(registered_pdn_ctx){
          mme_app_send_s11_delete_bearer_cmd(ue_context->privates.fields.mme_teid_s11, registered_pdn_ctx->s_gw_teid_s11_s4,
        		  (struct sockaddr *)&registered_pdn_ctx->s_gw_addr_s11_s4, &s10_handover_procedure->failed_ebi_list);
      }
    }
    OAILOG_FUNC_RETURN(LOG_MME_APP, RETURNok);
  }
  ...
  if(s11_procedure) {
	   ...
	   */
	  case MME_APP_S11_PROC_TYPE_CREATE_BEARER:
		  mme_app_send_s11_create_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11, REQUEST_REJECTED, (void*)s11_procedure->s11_trxn,
				  ((mme_app_s11_proc_create_bearer_t *)s11_procedure)->bcs_tbc);
		  break;
	  case MME_APP_S11_PROC_TYPE_UPDATE_BEARER:
		  mme_app_send_s11_update_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11, REQUEST_REJECTED, (void*)s11_procedure->s11_trxn,
				  ((mme_app_s11_proc_update_bearer_t*)s11_procedure)->bcs_tbu);
		  break;
	  case MME_APP_S11_PROC_TYPE_DELETE_BEARER:
		  mme_app_send_s11_delete_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11, REQUEST_REJECTED, (void*)s11_procedure->s11_trxn,
  ...
  if(registered_pdn_ctx){
	  mme_app_send_s11_delete_bearer_cmd(ue_session_pool->privates.fields.mme_teid_s11, registered_pdn_ctx->s_gw_teid_s11_s4,
			  (struct sockaddr *)&registered_pdn_ctx->s_gw_addr_s11_s4, &ebi_list);
  }
  OAILOG_FUNC_RETURN (LOG_MME_APP, RETURNok);
}
...
mme_app_trigger_paging_due_signaling(const mme_ue_s1ap_id_t ue_id){
  ...
  if(!emm_context){
	  OAILOG_ERROR (LOG_MME_APP, "No EMM data context exists for UE_ID " MME_UE_S1AP_ID_FMT ". \n", ue_context->privates.mme_ue_s1ap_id);
	  OAILOG_FUNC_RETURN (LOG_MME_APP, RETURNerror);
  }

  if(!emm_context->_tai_list.numberoflists){
	  OAILOG_WARNING(LOG_MME_APP, "EMM data context for UE_ID " MME_UE_S1AP_ID_FMT " and IMSI " IMSI_64_FMT "has no tai list. \n", ue_context->privates.mme_ue_s1ap_id,
			  ue_context->privates.fields.imsi);
  	  OAILOG_FUNC_RETURN (LOG_MME_APP, RETURNerror);
  }
  /** Check if the UE context is registered. */
  ...
  const itti_s11_release_access_bearers_response_t * const rel_access_bearers_rsp_pP)
{
  ...
  if (ue_context == NULL) {
    OAILOG_WARNING(LOG_MME_APP, "We didn't find this teid in list of UE: %" PRIX32 "\n", rel_access_bearers_rsp_pP->teid);
    OAILOG_FUNC_OUT (LOG_MME_APP);
  }
  ...
  #warning "LG to Dincer : you wanted to do something with this"
    ...
    itti_s11_create_bearer_request_t *  create_bearer_request_pP)
{
  ...
  if(!default_bc){
    ...
    mme_app_send_s11_create_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11, REQUEST_REJECTED, (void*)create_bearer_request_pP->trxn, create_bearer_request_pP->bearer_contexts);
    OAILOG_FUNC_OUT (LOG_MME_APP);
  }
  ...
  for(int num_bc = 0; num_bc < create_bearer_request_pP->bearer_contexts->num_bearer_context ; num_bc++){
    mme_app_get_session_bearer_context_from_all(ue_session_pool, create_bearer_request_pP->bearer_contexts->bearer_context[num_bc].eps_bearer_id, &bc_ded);
    if(bc_ded){
      OAILOG_ERROR(LOG_MME_APP, "Ded ebi %d already existing in list of UE with ueId " MME_UE_S1AP_ID_FMT" \n",
          create_bearer_request_pP->bearer_contexts->bearer_context[num_bc].eps_bearer_id, ue_session_pool->privates.mme_ue_s1ap_id);
      mme_app_send_s11_create_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11, REQUEST_REJECTED, (void*)create_bearer_request_pP->trxn, create_bearer_request_pP->bearer_contexts);
      OAILOG_FUNC_OUT (LOG_MME_APP);
    }
  }
  ...
  if(s11_proc_create_bearer) {
	  if(s11_proc_create_bearer->bcs_tbc->bearer_context[0].eps_bearer_id == create_bearer_request_pP->bearer_contexts->bearer_context[0].eps_bearer_id){
  	    OAILOG_WARNING(LOG_MME_APP, "Already found an create bearer procedure with the matching eBI %d for UE with ueId " MME_UE_S1AP_ID_FMT" with trx %lx. Update the transaction as %p. \n",
  	    		create_bearer_request_pP->bearer_contexts->bearer_context[0].eps_bearer_id, ue_session_pool->privates.mme_ue_s1ap_id, s11_proc_create_bearer->proc.s11_trxn, create_bearer_request_pP->trxn);
  	    /** Update the transaction. */
  	    s11_proc_create_bearer->proc.s11_trxn = (uintptr_t)create_bearer_request_pP->trxn;
	  } else {
		  s11_proc_create_bearer = NULL;
	  }
  }

  if(!s11_proc_create_bearer) {
	  s11_proc_create_bearer 							 = mme_app_create_s11_procedure_create_bearer(ue_session_pool);
	  /** Create an S11 procedure. */
	  if(!s11_proc_create_bearer){
  ...
  RB_FOREACH (registered_pdn_ctx, PdnContexts, &ue_session_pool->pdn_contexts) {
	  ...
	  if(!(bc_session->bearer_state & BEARER_STATE_ACTIVE)) {
		  OAILOG_INFO(LOG_MME_APP, "Bearer %d for UE " MME_UE_S1AP_ID_FMT" is in state %d. Not processing dedicated bearer requests until "
		  ...
		  if(mme_app_trigger_paging_due_signaling(ue_session_pool->privates.mme_ue_s1ap_id) == RETURNok){
			  OAILOG_INFO(LOG_MME_APP, "Successfully triggered paging for UE " MME_UE_S1AP_ID_FMT" due CBR. Not processing dedicated bearer requests until "
					  "session establishment has been completed successfully. \n", ue_session_pool->privates.mme_ue_s1ap_id);
			  /** Return back temporary reject due handover, such that it is triggered also in 2s. */
			  mme_app_send_s11_create_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11,
					  TEMP_REJECT_HO_IN_PROGRESS, (void*)create_bearer_request_pP->trxn, s11_proc_create_bearer->bcs_tbc);
			  /** Clear all cause values of the bearers. */
			  for(int numb = 0; numb < s11_proc_create_bearer->bcs_tbc->num_bearer_context; numb++) {
				  s11_proc_create_bearer->bcs_tbc->bearer_context[numb].cause.cause_value = 0;
			  }
			  OAILOG_FUNC_OUT (LOG_MME_APP);
		  } else {
			  OAILOG_INFO(LOG_MME_APP, "Could not trigger paging for UE " MME_UE_S1AP_ID_FMT" due CBR. Processing dedicated bearer requests (may need to set active-flag for idle-TAU). \n",
					  ue_session_pool->privates.mme_ue_s1ap_id);
			  break;
		  }
	  }
    ...
    itti_s11_update_bearer_request_t *  update_bearer_request_pP)
{
  ...
  for(int num_bearer = 0; num_bearer < update_bearer_request_pP->bearer_contexts->num_bearer_context; num_bearer++){
	bearer_context_new_t * ded_bc = NULL;
    mme_app_get_session_bearer_context_from_all(ue_session_pool, update_bearer_request_pP->bearer_contexts->bearer_context[num_bearer].eps_bearer_id, &ded_bc);
    if(!ded_bc || ded_bc->esm_ebr_context.status != ESM_EBR_ACTIVE){ /**< Status is active if it is idle TAU. */
      OAILOG_ERROR(LOG_MME_APP, "We could not find an (ACTIVE) dedicated bearer for ebi %d in bearer list of UE: " MME_UE_S1AP_ID_FMT". \n",
          update_bearer_request_pP->bearer_contexts->bearer_context[num_bearer].eps_bearer_id, ue_session_pool->privates.mme_ue_s1ap_id);
      mme_app_send_s11_update_bearer_rsp(ue_session_pool->privates.mme_ue_s1ap_id, ue_session_pool->privates.fields.saegw_teid_s11, 0, (void*)update_bearer_request_pP->trxn, update_bearer_request_pP->bearer_contexts);
      OAILOG_FUNC_OUT (LOG_MME_APP);
    }
  ...
  if((new_total_apn_ambr.br_dl += update_bearer_request_pP->apn_ambr.br_dl)  > ue_session_pool->privates.fields.subscribed_ue_ambr.br_dl){
    OAILOG_ERROR(LOG_MME_APP, "New total APN-AMBR (%" PRId64 ") exceeds the subscribed APN-AMBR (%" PRId64 ") (DL) for ueId " MME_UE_S1AP_ID_FMT". \n",
    		new_total_apn_ambr.br_dl,
			ue_session_pool->privates.fields.subscribed_ue_ambr.br_dl,
			ue_session_pool->privates.mme_ue_s1ap_id);
    mme_app_send_s11_update_bearer_rsp(update_bearer_request_pP->teid, ue_session_pool->privates.fields.saegw_teid_s11, 0, (void*)update_bearer_request_pP->trxn, update_bearer_request_pP->bearer_contexts);
    OAILOG_FUNC_OUT (LOG_MME_APP);
  }
  if((new_total_apn_ambr.br_ul += update_bearer_request_pP->apn_ambr.br_ul)  > ue_session_pool->privates.fields.subscribed_ue_ambr.br_ul){
    OAILOG_ERROR(LOG_MME_APP, "New total APN-AMBR (% " PRId64 ") exceeds the subscribed APN-AMBR (% " PRId64 ") (UL) for ueId " MME_UE_S1AP_ID_FMT". \n",
  ...
  if(s11_proc_update_bearer) {
	  if(s11_proc_update_bearer->bcs_tbu->bearer_context[0].eps_bearer_id == update_bearer_request_pP->bearer_contexts->bearer_context[0].eps_bearer_id){
  ...
  if(!s11_proc_update_bearer) {
	  s11_proc_update_bearer 							 = mme_app_create_s11_procedure_update_bearer(ue_session_pool);
	  ...
	  if(!(bc_session->bearer_state & BEARER_STATE_ACTIVE)) {
		  ...
		  if(mme_app_trigger_paging_due_signaling(ue_session_pool->privates.mme_ue_s1ap_id) == RETURNok){
			  OAILOG_INFO(LOG_MME_APP, "Successfully triggered paging for UE " MME_UE_S1AP_ID_FMT" due UBR. Not processing dedicated bearer requests until "
					  "session establishment has been completed successfully. \n", ue_session_pool->privates.mme_ue_s1ap_id);
			  mme_app_send_s11_update_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11,
					  TEMP_REJECT_HO_IN_PROGRESS, (void*)update_bearer_request_pP->trxn, s11_proc_update_bearer->bcs_tbu);
			  /** Clear all cause values of the bearers. */
			  for(int numb = 0; numb < s11_proc_update_bearer->bcs_tbu->num_bearer_context; numb++) {
				  s11_proc_update_bearer->bcs_tbu->bearer_context[numb].cause.cause_value = 0;
			  }
			  OAILOG_FUNC_OUT (LOG_MME_APP);
		  } else {
			  OAILOG_INFO(LOG_MME_APP, "Could not trigger paging for UE " MME_UE_S1AP_ID_FMT" due UBR. Processing dedicated bearer requests (may need to set active-flag for idle-TAU). \n",
					  ue_session_pool->privates.mme_ue_s1ap_id);
			  break;
		  }
	  }
    ...
    itti_s11_delete_bearer_request_t *  delete_bearer_request_pP)
{
  ...
  mme_app_s11_proc_delete_bearer_t* s11_proc_delete_bearer = mme_app_get_s11_procedure_delete_bearer(ue_session_pool);
  if(s11_proc_delete_bearer) {
	  if(s11_proc_delete_bearer->ebis.ebis[0] == delete_bearer_request_pP->ebi_list.ebis[0]){
  	    OAILOG_WARNING(LOG_MME_APP, "Already found a default bearer procedure with the matching eBI %d for UE with ueId " MME_UE_S1AP_ID_FMT" with trx %lx. Update the transaction as %p. \n",
  	    		delete_bearer_request_pP->ebi_list.ebis[0], ue_session_pool->privates.mme_ue_s1ap_id, s11_proc_delete_bearer->proc.s11_trxn, delete_bearer_request_pP->trxn);
  	    /** Update the transaction. */
  	    s11_proc_delete_bearer->proc.s11_trxn = (uintptr_t)delete_bearer_request_pP->trxn;
	  } else {
   	    s11_proc_delete_bearer = NULL;
	  }
  }

  if(!s11_proc_delete_bearer) {
	  s11_proc_delete_bearer 							 = mme_app_create_s11_procedure_delete_bearer(ue_session_pool);
	  /** Create an S11 procedure for the UBR. */
	  if(!s11_proc_delete_bearer){
	  ...
	  if(!(bc_session->bearer_state & BEARER_STATE_ACTIVE)) {
		  ...
		  if(mme_app_trigger_paging_due_signaling(ue_session_pool->privates.mme_ue_s1ap_id) == RETURNok){
			  OAILOG_INFO(LOG_MME_APP, "Successfully triggered paging for UE " MME_UE_S1AP_ID_FMT" due DBR. Not processing dedicated bearer requests until "
					  "session establishment has been completed successfully. \n", ue_session_pool->privates.mme_ue_s1ap_id);
			  mme_app_send_s11_delete_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11,
					  ue_session_pool->privates.fields.saegw_teid_s11, TEMP_REJECT_HO_IN_PROGRESS, (void*)delete_bearer_request_pP->trxn, &s11_proc_delete_bearer->ebis);
			  OAILOG_FUNC_OUT (LOG_MME_APP);
		  } else {
			  OAILOG_INFO(LOG_MME_APP, "Could not trigger paging for UE " MME_UE_S1AP_ID_FMT" due DBR. Processing dedicated bearer requests (may need to set active-flag for idle-TAU). \n",
					  ue_session_pool->privates.mme_ue_s1ap_id);
			  break;
		  }
	  }
...
void mme_app_delete_bearer_failure_indication (itti_s11_delete_bearer_failure_indication_t  * const delete_bearer_failure_ind){
  ...
  for(ebi_t num_ebi = 0; num_ebi < delete_bearer_failure_ind->bcs_failed.num_bearer_context; num_ebi++) {
    mme_app_release_bearer_context(ue_session_pool->privates.mme_ue_s1ap_id, NULL, EPS_BEARER_IDENTITY_UNASSIGNED,
        delete_bearer_failure_ind->bcs_failed.bearer_context[num_ebi].eps_bearer_id);
...
static void mme_app_handle_e_rab_setup_rsp_dedicated_bearer(const itti_s1ap_e_rab_setup_rsp_t * e_rab_setup_rsp)
{
  ...
  for (int i = 0; i < e_rab_setup_rsp->e_rab_setup_list.no_of_items; i++) {
    ...
    for(int num_bc = 0; num_bc < s11_proc_create_bearer->bcs_tbc->num_bearer_context; num_bc ++){
      if(s11_proc_create_bearer->bcs_tbc->bearer_context[num_bc].eps_bearer_id == e_rab_id){
        bc_tbc = &s11_proc_create_bearer->bcs_tbc->bearer_context[num_bc];
        break;
      }
    }
    if(!bc_tbc){
  ...
  if(s11_proc_create_bearer->proc.num_bearers_unhandled){
	...
	if(default_bc && (default_bc->bearer_state & BEARER_STATE_ACTIVE)){
	    ...
	    mme_app_send_s11_create_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11, 0, (void*)s11_proc_create_bearer->proc.s11_trxn, s11_proc_create_bearer->bcs_tbc);
  ...
  if(e_rab_setup_item) {
    ...
    if(bc_failed){
      ...
      if(((struct sockaddr*)&pdn_context->s_gw_addr_s11_s4)->sa_family != 0) {
        struct ue_context_s  *ue_context  = mme_ue_context_exists_mme_ue_s1ap_id (&mme_app_desc.mme_ue_contexts, ue_session_pool->privates.mme_ue_s1ap_id);
        mme_app_send_delete_session_request(ue_context, bc_failed->linked_ebi, (const struct sockaddr*)&pdn_context->s_gw_addr_s11_s4, ue_session_pool->privates.fields.saegw_teid_s11, true,
...
void mme_app_handle_e_rab_modify_rsp (itti_s1ap_e_rab_modify_rsp_t  * const e_rab_modify_rsp)
{
  ...
  for (int i = 0; i < e_rab_modify_rsp->e_rab_modify_list.no_of_items; i++) {
    ...
    for(int num_bc = 0; num_bc < s11_proc_update_bearer->bcs_tbu->num_bearer_context; num_bc ++){
      if(s11_proc_update_bearer->bcs_tbu->bearer_context[num_bc].eps_bearer_id == e_rab_id){
        bc_tbu = &s11_proc_update_bearer->bcs_tbu->bearer_context[num_bc];
        break;
      }
    }
    if(!bc_tbu){
    ...
    if(bc_tbu->cause.cause_value != 0 && bc_tbu->cause.cause_value != REQUEST_ACCEPTED){
      OAILOG_DEBUG (LOG_MME_APP, "The ebi %d has already a negative error cause %d for ueId : " MME_UE_S1AP_ID_FMT "\n", bc_tbu->eps_bearer_id, bc_tbu->cause.cause_value, e_rab_modify_rsp->mme_ue_s1ap_id);
      continue;
    }
    /** Cause is either not set or negative. It must be in the session bearers. */
    bc_success = mme_app_get_session_bearer_context(pdn_context, e_rab_id);
...
#warning "LG: mme_app_release_bearer_context() arg 2 pdn_cid_t or context_identifier_t ?"
  ...
  if(s11_proc_update_bearer->proc.num_bearers_unhandled){
    ...
    if(default_bc && (default_bc->bearer_state & BEARER_STATE_ACTIVE)){
    	OAILOG_DEBUG (LOG_MME_APP, "For the S11 update bearer procedure, no pending bearers left (either success or failure) & default bearer activated for UE " MME_UE_S1AP_ID_FMT ". "
    			"Sending UBResp immediately. \n", ue_session_pool->privates.mme_ue_s1ap_id);
        mme_app_send_s11_update_bearer_rsp(ue_session_pool->privates.fields.mme_teid_s11, ue_session_pool->privates.fields.saegw_teid_s11, 0, (void*)s11_proc_update_bearer->proc.s11_trxn, s11_proc_update_bearer->bcs_tbu);
        /*
...
void mme_app_handle_e_rab_release_ind (const itti_s1ap_e_rab_release_ind_t   * const e_rab_release_ind) {
  ...
  if(ebi_list.num_ebi){
    ...
    if(pdn_context){
      /** Trigger a Delete Bearer Command. */
      mme_app_send_s11_delete_bearer_cmd(ue_session_pool->privates.fields.mme_teid_s11, pdn_context->s_gw_teid_s11_s4, (struct sockaddr *)&pdn_context->s_gw_addr_s11_s4, &ebi_list);
      OAILOG_FUNC_OUT (LOG_MME_APP);
    }
  }
...
void mme_app_handle_activate_eps_bearer_ctx_cnf (itti_nas_activate_eps_bearer_ctx_cnf_t   * const activate_eps_bearer_ctx_cnf)
{
  ...
  bearer_context_to_be_created_t * bc_tbc = NULL;
  for(int num_bc = 0; num_bc < s11_proc_create_bearer->bcs_tbc->num_bearer_context; num_bc++){
    if(s11_proc_create_bearer->bcs_tbc->bearer_context[num_bc].s1u_sgw_fteid.teid == activate_eps_bearer_ctx_cnf->saegw_s1u_teid){
      bc_tbc = &s11_proc_create_bearer->bcs_tbc->bearer_context[num_bc];
      break;
    }
  }
...
void mme_app_handle_activate_eps_bearer_ctx_rej (itti_nas_activate_eps_bearer_ctx_rej_t   * const activate_eps_bearer_ctx_rej)
{
  ...
  bearer_context_to_be_created_t * bc_tbc = NULL;
  for(int num_bc = 0; num_bc < s11_proc_create_bearer->bcs_tbc->num_bearer_context; num_bc++ ){
    if(s11_proc_create_bearer->bcs_tbc->bearer_context[num_bc].s1u_sgw_fteid.teid == activate_eps_bearer_ctx_rej->saegw_s1u_teid){
      bc_tbc = &s11_proc_create_bearer->bcs_tbc->bearer_context[num_bc];
      break;
    }
  }
...
void mme_app_handle_modify_eps_bearer_ctx_cnf (itti_nas_modify_eps_bearer_ctx_cnf_t   * const modify_eps_bearer_ctx_cnf)
{
  ...
  bearer_context_to_be_updated_t * bc_tbu = NULL;
  for(int num_bc = 0; num_bc < s11_proc_update_bearer->bcs_tbu->num_bearer_context; num_bc ++){
    if(s11_proc_update_bearer->bcs_tbu->bearer_context[num_bc].eps_bearer_id == modify_eps_bearer_ctx_cnf->ebi){
      bc_tbu = &s11_proc_update_bearer->bcs_tbu->bearer_context[num_bc];
      break;
    }
  }
...
void mme_app_handle_modify_eps_bearer_ctx_rej (itti_nas_modify_eps_bearer_ctx_rej_t   * const modify_eps_bearer_ctx_rej)
{
  ...
  bearer_context_to_be_updated_t * bc_tbu = NULL;
  for(int num_bc = 0; num_bc < s11_proc_update_bearer->bcs_tbu->num_bearer_context; num_bc++ ){
    if(s11_proc_update_bearer->bcs_tbu->bearer_context[num_bc].eps_bearer_id == modify_eps_bearer_ctx_rej->ebi){
      bc_tbu = &s11_proc_update_bearer->bcs_tbu->bearer_context[num_bc];
      break;
    }
  }
...
//  forward_relocation_request_p->imsi.length = strlen ((const char *)forward_relocation_request_p->imsi.u.value);
   ...
   */
  s10_handover_procedure->source_enb_ue_s1ap_id = ue_context->privates.fields.enb_ue_s1ap_id;
  s10_handover_procedure->target_enb_ue_s1ap_id = INVALID_MME_UE_S1AP_ID;
  s10_handover_procedure->source_ecgi = ue_context->privates.fields.e_utran_cgi;
...
// }
 ...
 /** Set the source ENB as invalid. */
 s10_proc_mme_handover->source_enb_ue_s1ap_id = INVALID_MME_UE_S1AP_ID;
    ...
    )
{
   ...
   */
  bearer_id = forward_relocation_response_pP->handovered_bearers.bearer_context[0].eps_bearer_id /* - 5 */ ;
...
// todo: DevAssert(ue_context->privates.fields.ecm_state == ECM_CONNECTED); /**< Any timeouts here should erase the context, not change the signaling state back to IDLE but leave the context. */
  OAILOG_INFO(LOG_MME_APP, "Sent S1AP MME Status transfer to the target eNodeB %d for UE with enb_ue_s1ap_id: " ENB_UE_S1AP_ID_FMT", mme_ue_s1ap_id. "MME_UE_S1AP_ID_FMT ". \n",
    ...
    )
{
 ...
 OAILOG_INFO(LOG_MME_APP, "Received S1AP_HANDOVER_FAILURE from target eNB for UE_ID " MME_UE_S1AP_ID_FMT ". \n", handover_failure_pP->mme_ue_s1ap_id);
...
// forward_relocation_complete_acknowledge_p->peer_ip = s10_handover_proc->proc.peer_ip; /**< Set the target TEID. */
 ...
 if(old_ue_reference){
   ...
   if (timer_setup (mme_config.mme_mobility_completion_timer, 0,
     ...
     old_ue_reference->s1ap_handover_completion_timer.id = S1AP_TIMER_INACTIVE_ID;
     s10_handover_proc->proc.timer.id = MME_APP_TIMER_INACTIVE_ID;
   } else {
