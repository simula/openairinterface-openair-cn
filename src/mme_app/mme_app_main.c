...
static void _check_mme_healthy_and_notify_service303 (void);
  ...
  void *args)
{
  ...
  while (1) {
    ...
    case S11_PAGING_REQUEST:{
        ...
        OAILOG_DEBUG(TASK_MME_APP, "MME handling paging request for IMSI%s\n",
                     imsi);
        if (mme_app_handle_initial_paging_request(imsi) != RETURNok) {
          OAILOG_ERROR(TASK_MME_APP,
    ...
    case TIMER_HAS_EXPIRED:{
        ...
        if (!timer_exists (received_message_p->ittiMsg.timer_has_expired.timer_id)) {
          OAILOG_WARNING (LOG_MME_APP, "Timer expiry signal received for timer \
        ...
        if (received_message_p->ittiMsg.timer_has_expired.timer_id == mme_app_desc.statistic_timer_id) {
          ...
          if (received_message_p->ittiMsg.timer_has_expired.timer_id == ue_context_p->mobile_reachability_timer.id) {
            ...
            increment_counter ("initial_context_setup_request_timer_expired", 1, NO_LABELS);
            mme_app_handle_initial_context_setup_rsp_timer_expiry (ue_context_p);
          } else if (received_message_p->ittiMsg.timer_has_expired.timer_id == ue_context_p->paging_response_timer.id) {
            mme_app_handle_paging_timer_expiry (ue_context_p);
          } else {
            OAILOG_WARNING (LOG_MME_APP, "Timer expired but no associated timer_id for UE id %d\n", mme_ue_s1ap_id);
          }
        }
    ...
    case ACTIVATE_MESSAGE:{
        ...
        _check_mme_healthy_and_notify_service303();
      }
      break;

    case SCTP_MME_SERVER_INITIALIZED:{
        mme_sctp_bounded = &received_message_p
          ->ittiMsg.sctp_mme_server_initialized.successful;
        _check_mme_healthy_and_notify_service303();
      }
      break;
  ...
  const mme_config_t * mme_config_p)
{
  ...
  mme_app_desc.mme_ue_contexts.guti_ue_context_htbl = obj_hashtable_ts_create (mme_config.max_ues, NULL, NULL, hash_free_int_func, b);
  bdestroy(b);
   ...
   */
  if (timer_setup (mme_config_p->mme_statistic_timer, 0, TASK_MME_APP, INSTANCE_DEFAULT, TIMER_PERIODIC, NULL, 0, &mme_app_desc.statistic_timer_id) < 0) {
...
static void
_check_mme_healthy_and_notify_service303 (void)
{
