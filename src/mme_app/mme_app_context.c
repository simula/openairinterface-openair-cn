  ...
  ecm_state_t new_ecm_state)
{
  ...
  if (new_ecm_state == ECM_IDLE)
  {
    ...
    if (mme_config.nas_config.t3412_min > 0) {
      ...
      if (timer_setup (ue_context_p->mobile_reachability_timer.sec, 0, TASK_MME_APP, INSTANCE_DEFAULT, TIMER_ONE_SHOT, (void *)&(ue_context_p->mme_ue_s1ap_id), sizeof(mme_ue_s1ap_id_t), &(ue_context_p->mobile_reachability_timer.id)) < 0) {
    ...
    if (ue_context_p->ecm_state == ECM_CONNECTED) {
      ...
      OAILOG_INFO (LOG_MME_APP, "UE STATE - IDLE. IMSI = " IMSI_64_FMT "\n", ue_context_p->imsi);
    }
  ...
  }else if ((ue_context_p->ecm_state == ECM_IDLE) && (new_ecm_state == ECM_CONNECTED))
  {
    ...
    OAILOG_INFO (LOG_MME_APP, "UE STATE - CONNECTED. IMSI = " IMSI_64_FMT "\n", ue_context_p->imsi);
  }
  return;
}
//------------------------------------------------------------------------------
  ...
  mme_ue_s1ap_id_t       mme_ue_s1ap_id, mm_state_t  new_mm_state)
{
  ...
  if ((ue_context_p->mm_state == UE_UNREGISTERED) && (new_mm_state == UE_REGISTERED))
  {
    ...
    OAILOG_INFO (LOG_MME_APP, "UE STATE - REGISTERED. IMSI = " IMSI_64_FMT "\n", ue_context_p->imsi);
  } else if ((ue_context_p->mm_state == UE_REGISTERED) && (new_mm_state == UE_UNREGISTERED))
  {
    ...
    OAILOG_INFO (LOG_MME_APP, "UE STATE - UNREGISTERED. IMSI = " IMSI_64_FMT "\n", ue_context_p->imsi);
  }
  OAILOG_FUNC_OUT (LOG_MME_APP);
}
