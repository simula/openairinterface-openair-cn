...
void fillscefnotify(
    DataAccess& dataaccess, DAEventIdList& event_list, uint32_t evt_type,
    std::vector<std::pair<std::string, uint32_t>>& scef_tonotify);

void fdJsonError(const char* err) { printf("json error: %s", err); }
...
IDRRreq::IDRRreq(Application& app, FDMessageRequest* cir_req,
                 EvenStatusMap* evt_map, RIRBuilder* rirbuilder,
                 std::string& imsi, std::string& msisdn)
    : INSDRreq(app),
      cir_req(cir_req),
      evt_map(evt_map),
...
void IDRRreq::processAnswer(FDMessageAnswer& ans) {
  ...
  if (ida.result_code.get(ida_result_code)) {
    ...
    StatsHss::singleton().registerStatResult(stat_hss_idr, vendor_code,
                                             ida_result_code);
  }
  ...
  if (!m_rirbuilder) {
    ...
    // Fill the long term event result status based on the insertion result on
    // db
    for (EvenStatusMap::iterator it = evt_map->begin(); it != evt_map->end();
         ++it) {
      if (it->second.isLongTermEvt()) {
        FDAvp mon_evt_cfg_status(
            s6tApp->getDict().avpMonitoringEventConfigStatus());
    ...
    if (ida_result_code == DIAMETER_SUCCESS) {
      ...
      for (std::list<MonitoringEventConfigStatusExtractor*>::iterator it =
               ida.monitoring_event_config_status.getList().begin();
        ...
        EvenStatusMap::iterator map_iter =
            evt_map->find(std::make_pair(scef_id, scef_ref_id));
        ...
        if (map_iter != evt_map->end()) {
          if (!map_iter->second.isLongTermEvt()) {
            ...
            if (service_report_found &&
                (*it_service)
                    ->service_result.service_result_code.get(
                        service_result_code)) {
              Logger::s6as6d().info(
                  "service_result found on the mme response, updating to %u",
                  service_result_code);
    ...
    StatsHss::singleton().registerStatResult(stat_hss_cir, 0,
                                             ER_DIAMETER_SUCCESS);
    ...
    if (ida_result_code == DIAMETER_SUCCESS) {
      ...
      for (std::list<MonitoringEventConfigStatusExtractor*>::iterator it =
        ...
        mme_response->insert(
            std::make_pair(std::make_pair(scef_id, scef_ref_id), mme_status));
      }
      HandleMmeResponseEvtMsg* e =
          new HandleMmeResponseEvtMsg(mme_response, m_imsi, 0, m_msisdn);
      ...
      HandleMmeResponseEvtMsg* e =
          new HandleMmeResponseEvtMsg(NULL, m_imsi, MME_DOWN, m_msisdn);
      m_rirbuilder->postMessage(e);
    }
  }
}
...
int Application::sendINSDRreq(
    s6t::MonitoringEventConfigurationExtractorList& cir_monevtcfg,
  ...
  if (!imsi_attached || !mme_reachable) {
    if (rir_builder == NULL) {
      ...
      if (!imsi_attached) {
        printf(
            "****************Application::sendINSDRreq::IMSI_NOT_ACTIVE\n\n");
      ...
      if (!imsi_attached) {
        printf(
            "****************Application::sendINSDRreq::POSTING fake "
            "IMSI_NOT_ACTIVE\n\n");
        HandleMmeResponseEvtMsg* e = new HandleMmeResponseEvtMsg(
            NULL, imsi, IMSI_NOT_ACTIVE, imsi_info.str_msisdn);
        ...
        printf(
            ...
            "MME_DOWN\n\n");
        HandleMmeResponseEvtMsg* e = new HandleMmeResponseEvtMsg(
            NULL, imsi, MME_DOWN, imsi_info.str_msisdn);
        rir_builder->postMessage(e);
      }
      return true;
    }
  ...
  INSDRreq* s = createINSDRreq(cir_monevtcfg, imsi, cir_req, evt_map, imsi_info,
                               rir_builder);
...
INSDRreq* Application::createINSDRreq(
    s6t::MonitoringEventConfigurationExtractorList& cir_monevtcfg,
    std::string& imsi, FDMessageRequest* cir_req, EvenStatusMap* evt_map,
    DAImsiInfo& imsi_info, RIRBuilder* rir_builder) {
  //  creates the INSDRreq object
  INSDRreq* s = new IDRRreq(*this, cir_req, evt_map, rir_builder, imsi,
                            imsi_info.str_msisdn);
...

  {
    ...
    FDAvp subscription_data(getDict().avpSubscriptionData(),
                            (struct avp*)idr.subscription_data.getReference());
...
int PUURcmd::process(FDMessageRequest* req) {
  ...
  if (DIAMETER_ERROR_IS_VENDOR(result_code) && experimental) {
    ...
    experimental_result.add(m_app.getDict().avpExperimentalResultCode(),
                            result_code);
    ans.add(experimental_result);
    StatsHss::singleton().registerStatResult(stat_hss_pur, VENDOR_3GPP,
                                             result_code);
  } else {
...
ULRStateProcessor::ULRStateProcessor(uint16_t state, ULRProcessor* ulrproc)
    : m_state(state), m_processor(ulrproc) {}
...
ULRProcessor::ULRProcessor(FDMessageRequest& req, s6as6d::Application& app,
                           s6as6d::Dictionary& dict)
    : m_ulr(req, dict), m_ans(&req), m_app(app), m_dict(dict) {
  m_perf_timer = 0;
  m_present_flags = 0;
...
void ULRProcessor::on_ulr_callback(CassFuture* future, void* data) {
  ...
  const char* actions[] = {
      "ULRDB_GET_IMSI_INFO",      "ULRDB_GET_EXT_IDS",
      "ULRDB_GET_EVNTIDS_MSISDN", "ULRDB_GET_EVNTIDS_EXTIDS",
      "ULRDB_GET_EVNTS_EVNTIDS",  "ULRDB_GET_MMEID_HOST",
      "ULRDB_UPDATE_IMSI",        "UNKNOWN"};
  int actionofs =
      action->getAction() == ULRDB_GET_IMSI_INFO
          ? 0
          : action->getAction() == ULRDB_GET_EXT_IDS
                ? 1
                : action->getAction() == ULRDB_GET_EVNTIDS_MSISDN
                      ? 2
                      : action->getAction() == ULRDB_GET_EVNTIDS_EXTIDS
                            ? 3
                            : action->getAction() == ULRDB_GET_EVNTS_EVNTIDS
                                  ? 4
                                  : action->getAction() == ULRDB_GET_MMEID_HOST
                                        ? 5
                                        : action->getAction() ==
                                                  ULRDB_UPDATE_IMSI
                                              ? 6
  ...
  if (l.acquire(false)) action->getProcessor().triggerNextPhase();
...
void ULRProcessor::triggerNextPhase() {
  ...
  fdHss.getWorkMgr().addWork(new WorkerMessage(
      WORKER_EVENT, new ULRStateProcessor(m_nextphase, this)));
}

bool ULRProcessor::phaseReady(int phase, uint32_t adjustment) {
  bool ready = false;
#ifdef TRACK_EXECUTION
  const char* phases[] = {"ULRSTATE_PHASEFINAL",
                          "ULRSTATE_PHASE1",
                          "ULRSTATE_PHASE2",
                          "ULRSTATE_PHASE3",
                          "ULRSTATE_PHASE4",
  ...
  switch (phase) {
    ...
    case ULRSTATE_PHASEFINAL: {
      ...
      printf("%lld,%s,%p,%s,m_dbissued=%d,m_msgissued=%d\n",
             STIMER_GET_CURRENT_TIME, __PRETTY_FUNCTION__, this,
             phases[phase - ULRSTATE_BASE], m_dbissued - adjustment,
             m_msgissued);
#endif
...

  {
    ...
    while (pthis && pthis->phaseReady(pthis->m_nextphase)) {
      ...
      switch (pthis->m_nextphase) {
        ...
        default: {
          Logger::s6as6d().warn("Unrecognized ULRSTATE (%u)",
                                pthis->m_nextphase);
          pthis = NULL;
          break;
        }
...
void ULRProcessor::getEventIdsMsisdn(SCassFuture& future) {
...

  {
    ...
    bool success =
        m_app.dataaccess().getEventIdsFromMsisdnData(future, m_evtIdLst);
...
void ULRProcessor::getEventIdsExternalIds(SCassFuture& future) {
...

  {
    SMutexLock l(m_lstmutex);
    bool success =
        m_app.dataaccess().getEventIdsFromExtIdsData(future, m_evtIdLst);
    DB_OP_COMPLETE(ULRDB_GET_EVNTIDS_MSISDN, m_dbexecuted, m_dbresult, success);
...
void ULRProcessor::getEvents() {
  ...
  for (auto it = m_evtIdLst.begin(); it != m_evtIdLst.end(); ++it) {
    if (scef_id != (*it)->scef_id && scef_ref_ids.size() > 0) {
      // issue the query
      atomic_inc_fetch(m_dbissued);
      success = m_app.dataaccess().getEvents(
          scef_id.c_str(), scef_ref_ids, m_evtLst, on_ulr_callback,
          new ULRDatabaseAction(ULRDB_GET_EVNTS_EVNTIDS, *this));
      if (!success) {
        DB_OP_COMPLETE(ULRDB_GET_EVNTS_EVNTIDS, m_dbexecuted, m_dbresult,
                       false);
        atomic_dec_fetch(m_dbissued);
        return;
      }
...
void ULRProcessor::updateImsiInfo(SCassFuture& future) {
  ...
  if (!success)
    Logger::s6as6d().error(
        "DataAccess::%s - Error %d executing updateImsiInfo()", __func__,
        future.errorCode());
...
void ULRProcessor::phase1() {
  ...
  if (m_new_info.imsi.length() > IMSI_LENGTH) {
    ...
    m_ans.send();
    StatsHss::singleton().registerStatResult(stat_hss_ulr, 0,
  ...
  if (result) {
    ...
    if (!result) {
      ...
      DB_OP_COMPLETE(ULRDB_GET_EVNTIDS_EXTIDS, m_dbexecuted, m_dbresult,
                     result);
  ...
  if (result) {
    ...
    if (!result) {
      DB_OP_COMPLETE(ULRDB_GET_EVNTIDS_MSISDN, m_dbexecuted, m_dbresult,
                     result);
      DB_OP_COMPLETE(ULRDB_GET_EVNTS_EVNTIDS, m_dbexecuted, m_dbresult, result);
      atomic_dec_fetch(m_dbissued);
    }
  ...
  if (result) {
    ...
    m_ans.add(er);
    m_ans.send();
    StatsHss::singleton().registerStatResult(stat_hss_ulr, VENDOR_3GPP,
...
void ULRProcessor::phase2() {
  ...
  if (FLAG_IS_SET(u32, ULR_INITIAL_ATTACH_IND)) {
    ...
    if (!m_orig_info.mmehost.empty() && !m_orig_info.mmerealm.empty()) {
      if (m_orig_info.mmehost != m_new_info.mmehost) {
        FDAvp er(m_dict.avpExperimentalResult());
        er.add(m_dict.avpVendorId(), VENDOR_3GPP);
        er.add(m_dict.avpExperimentalResultCode(),
               DIAMETER_ERROR_UNKNOWN_SERVING_NODE);
        m_ans.add(er);
        m_ans.send();
        StatsHss::singleton().registerStatResult(
            stat_hss_ulr, VENDOR_3GPP, DIAMETER_ERROR_UNKNOWN_SERVING_NODE);
        m_nextphase = ULRSTATE_PHASEFINAL;
        return;
      }
  ...
  for (std::list<s6as6d::SupportedFeaturesExtractor*>::iterator seit =
           m_ulr.supported_features.getList().begin();
       seit != m_ulr.supported_features.getList().end(); ++seit) {
  ...
  if (!FLAG_IS_SET(m_ulrflags, ULR_SKIP_SUBSCRIBER_DATA)) {
    ...
    if (fdJsonAddAvps(m_orig_info.subscription_data.c_str(), m_ans.getMsg(),
      ...
      m_ans.send();
      StatsHss::singleton().registerStatResult(
          stat_hss_ulr, VENDOR_3GPP, DIAMETER_ERROR_UNKNOWN_EPS_SUBSCRIPTION);
...
void ULRProcessor::phase3() {
  if (!FLAG_IS_SET(m_ulrflags, ULR_SKIP_SUBSCRIBER_DATA)) {
    ...
    if (!m_evtLst.empty()) {
      ...
      FDAvp sd(m_dict.avpSubscriptionData(),
               (struct avp*)ula.subscription_data.getReference(), false);
      for (DAEventList::iterator it = m_evtLst.begin(); it != m_evtLst.end();
           ++it) {
  ...
  try {
    // After sending the ula, we verify if we need to send a RIR reporting the
    // change of pmnid
      ...
      if (m_orig_info.visited_plmnid != m_new_info.visited_plmnid) {
        for (DAEventList::iterator it_evt = m_evtLst.begin();
          ...
          if ((*it_evt)->monitoring_type == ROAMING_STATUS_EVT) {
            ...
            s->add(fdHss.gets6tApp()->getDict().avpSessionId(),
                   s->getSessionId());
            s->add(fdHss.gets6tApp()->getDict().avpAuthSessionState(), 1);
            s->addOrigin();

            s->add(fdHss.gets6tApp()->getDict().avpDestinationHost(),
                   (*it_evt)->scef_id);

            size_t afound = (*it_evt)->scef_id.find(".");
            if (afound != std::string::npos) {
              s->add(fdHss.gets6tApp()->getDict().avpDestinationRealm(),
                     (*it_evt)->scef_id.substr(afound + 1));
            }

            FDAvp monitoring_event_report(
                fdHss.gets6tApp()->getDict().avpMonitoringEventReport());

            monitoring_event_report.add(
                fdHss.gets6tApp()->getDict().avpScefId(), (*it_evt)->scef_id);
            monitoring_event_report.add(
                fdHss.gets6tApp()->getDict().avpScefReferenceId(),
                (*it_evt)->scef_id);
            monitoring_event_report.add(
                fdHss.gets6tApp()->getDict().avpMonitoringType(),
                ROAMING_STATUS_EVT);
            monitoring_event_report.add(
                fdHss.gets6tApp()->getDict().avpVisitedPlmnId(), m_plmn_id,
                m_plmn_len);
            ...
            try {
              ...
              Logger::s6as6d().error(
                  "EXCEPTION while sending a RIR for ROAMING_STATUS_EVT - %s",
                  ex.what());
              delete s;
              s = NULL;
            }
...
void ULRProcessor::phase5() { m_nextphase = ULRSTATE_PHASEFINAL; }

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

AIRStateProcessor::AIRStateProcessor(uint16_t state, AIRProcessor* airproc)
    : m_state(state), m_processor(airproc) {}
...
AIRProcessor::AIRProcessor(FDMessageRequest& req, s6as6d::Application& app,
                           s6as6d::Dictionary& dict)
    : m_air(req, dict), m_ans(&req), m_app(app), m_dict(dict) {
  m_uimsi = 0;
  m_num_vectors = 0;
  m_plmn_len = sizeof(m_plmn_id);
...
#ifdef TRACK_EXECUTION
  const char* actions[] = {"AIRDB_GET_IMSI_SEC", "AIRDB_UPDATE_IMSI",
                           "UNKNOWN"};
  int actionofs = action->getAction() == AIRDB_GET_IMSI_SEC
                      ? 0
...
void AIRProcessor::triggerNextPhase() {
  atomic_inc_fetch(m_msgissued);
  fdHss.getWorkMgr().addWork(new WorkerMessage(
      WORKER_EVENT, new AIRStateProcessor(m_nextphase, this)));
}

bool AIRProcessor::phaseReady(int phase, uint32_t adjustment) {
  bool ready = false;
#ifdef TRACK_EXECUTION
  static const char* phases[] = {"AIRSTATE_PHASEFINAL", "AIRSTATE_PHASE1",
                                 "AIRSTATE_PHASE2", "AIRSTATE_PHASE3",
                                 "UNKNOWN"};
#endif
  ...
  switch (phase) {
    ...
    case AIRSTATE_PHASEFINAL: {
      ...
      printf("%lld,%s,%p,%s,m_dbissued=%d,m_msgissued=%d\n",
             STIMER_GET_CURRENT_TIME, __PRETTY_FUNCTION__, this,
             phases[phase - AIRSTATE_BASE], m_dbissued - adjustment,
             m_msgissued);
#endif
      ready = ((m_dbissued - adjustment) <= 0 && m_msgissued == 0);
      break;
...
#ifdef TRACK_EXECUTION
      printf("%lld,%s,%p,%s\n", STIMER_GET_CURRENT_TIME, __PRETTY_FUNCTION__,
             pthis, phases[pthis->m_nextphase - AIRSTATE_BASE]);
#endif
      switch (pthis->m_nextphase) {
        case AIRSTATE_PHASE1: {
          pthis->phase1();
        ...
        default: {
          Logger::s6as6d().warn("Unrecognized AIRSTATE (%u)",
                                pthis->m_nextphase);
          pthis = NULL;
          break;
        }
...
void AIRProcessor::updateImsi(SCassFuture& future) {
  ...
  if (!success) {
    Logger::system().warn(
        "AIRProcessor::%s - Error %d while executing updateRandSqn()", __func__,
        future.errorCode());
  }
}
...
void AIRProcessor::phase1() {
  ...
  if (m_imsi.length() > IMSI_LENGTH) {
    m_ans.add(m_dict.avpResultCode(), ER_DIAMETER_INVALID_AVP_VALUE);
    m_ans.send();
    StatsHss::singleton().registerStatResult(stat_hss_air, 0,
                                             ER_DIAMETER_INVALID_AVP_VALUE);
    m_nextphase = ULRSTATE_PHASEFINAL;
    return;
  }
  ...
  if (m_air.requested_eutran_authentication_info.number_of_requested_vectors
          .get(m_num_vectors)) {
  ...
  if (!eutran_avp_found) {
    if (m_air.requested_utran_geran_authentication_info
            .number_of_requested_vectors.get(m_num_vectors)) {
      FDAvp er(m_dict.avpExperimentalResult());
      er.add(m_dict.avpVendorId(), VENDOR_3GPP);
      er.add(m_dict.avpExperimentalResultCode(),
             DIAMETER_ERROR_RAT_NOT_ALLOWED);
      m_ans.add(er);
      m_ans.send();
      StatsHss::singleton().registerStatResult(stat_hss_air, VENDOR_3GPP,
                                               DIAMETER_ERROR_RAT_NOT_ALLOWED);
      m_nextphase = ULRSTATE_PHASEFINAL;
      return;
    }
    ...
    if (m_plmn_len == 3) {
      if (!Options::getroamallow()) {
        if (apply_access_restriction((char*)m_imsi.c_str(), m_plmn_id) != 0) {
          ...
          m_ans.send();
          StatsHss::singleton().registerStatResult(
              stat_hss_air, VENDOR_3GPP, DIAMETER_ERROR_ROAMING_NOT_ALLOWED);
          m_nextphase = ULRSTATE_PHASEFINAL;
          return;
        }
      }
...
void AIRProcessor::phase2() {
  ...
  if (m_auts_set) {
    ...
    if (sqn != NULL) {
      ...
      SqnU64Union eu;
      SQN_TO_U64(m_sec.sqn, eu);
      std::cerr << "Could not resync " << m_uimsi << " HSS SQN " << eu.u64
                << std::endl;
    }
  }
  ...
  for (uint32_t i = 0; i < m_num_vectors; i++) {
    ...
    generate_vector_cpp(m_sec.opc, m_uimsi, m_sec.key, m_plmn_id, m_sec.sqn,
                        &m_vector[i]);
  }
  ...
  for (uint32_t i = 0; i < m_num_vectors; i++) {
    ...
    eurtran_vector.add(m_dict.avpRand(), m_vector[i].rand,
                       sizeof(m_vector[i].rand));
    eurtran_vector.add(m_dict.avpXres(), m_vector[i].xres,
                       sizeof(m_vector[i].xres));
    eurtran_vector.add(m_dict.avpAutn(), m_vector[i].autn,
                       sizeof(m_vector[i].autn));
    eurtran_vector.add(m_dict.avpKasme(), m_vector[i].kasme,
                       sizeof(m_vector[i].kasme));
...
void AIRProcessor::phase3() { m_nextphase = AIRSTATE_PHASEFINAL; }
