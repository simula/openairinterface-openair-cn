  ...
  nw_gtpv2c_ulp_api_t * pUlpApi)
{
  ...
  MSC_LOG_RX_MESSAGE (MSC_S11_MME, MSC_SGW, NULL, 0, "0 MODIFY_BEARER_RESPONSE local S11 teid " TEID_FMT " cause %u",
    resp_p->teid, resp_p->cause);
  rc = nwGtpv2cMsgParserDelete (*stack_p, pMsgParser);
  DevAssert (NW_OK == rc);
  rc = nwGtpv2cMsgDelete (*stack_p, (pUlpApi->hMsg));
...
//------------------------------------------------------------------------------
int
s11_mme_handle_create_bearer_request (
  nw_gtpv2c_stack_handle_t * stack_p,
  nw_gtpv2c_ulp_api_t * pUlpApi)
{
  nw_rc_t                                   rc = NW_OK;
  ...
  nw_gtpv2c_ulp_api_t * pUlpApi)
{
  ...
  uint8_t                                 offendingIeType,
                                          offendingIeInstance;
  uint16_t                                offendingIeLength;
  itti_s11_delete_bearer_request_t        *req_p;
  MessageDef                              *message_p;
  nw_gtpv2c_msg_parser_t                     *pMsgParser;

  DevAssert (stack_p );
  message_p = itti_alloc_new_message (TASK_S11, S11_DELETE_BEARER_REQUEST);

  if (message_p) {
    req_p = &message_p->ittiMsg.s11_delete_bearer_request;
     ...
     * Create a new message parser
     */
    rc = nwGtpv2cMsgParserNew (*stack_p, NW_GTP_DELETE_BEARER_REQ, s11_ie_indication_generic, NULL, &pMsgParser);
    DevAssert (NW_OK == rc);
    ...
    if (rc != NW_OK) {
      MSC_LOG_RX_DISCARDED_MESSAGE (MSC_S11_MME, MSC_SGW, NULL, 0, "0 DELETE_BEARER_REQUEST local S11 teid " TEID_FMT " ", req_p->teid);
      /*
       * TODO: handle this case
       */
      itti_free (ITTI_MSG_ORIGIN_ID (message_p), message_p);
    ...
    MSC_LOG_RX_MESSAGE (MSC_S11_MME, MSC_SGW, NULL, 0, "0 DELETE_BEARER_REQUEST local S11 teid " TEID_FMT " lebi %u",
        req_p->teid, req_p->linked_eps_bearer_id);
    rc = nwGtpv2cMsgParserDelete (*stack_p, pMsgParser);
    DevAssert (NW_OK == rc);
    rc = nwGtpv2cMsgDelete (*stack_p, (pUlpApi->hMsg));
...
s11_mme_delete_bearer_response (
  nw_gtpv2c_stack_handle_t * stack_p,
  itti_s11_delete_bearer_response_t * response_p)
{
  gtpv2c_cause_t                           cause;
  nw_rc_t                                   rc;
  nw_gtpv2c_ulp_api_t                         ulp_req;
  ...
  ulp_req.apiType = NW_GTPV2C_ULP_API_TRIGGERED_RSP;
  ulp_req.u_api_info.triggeredRspInfo.hTrxn = trxn;
  rc = nwGtpv2cMsgNew (*stack_p, true, NW_GTP_DELETE_BEARER_RSP, response_p->teid, 0, &(ulp_req.hMsg));
  DevAssert (NW_OK == rc);
  /*
   * Set the remote TEID
   */
  ...
  MSC_LOG_TX_MESSAGE (MSC_S11_MME, MSC_SGW, NULL, 0, "0 DELETE_BEARER_RESPONSE S11 teid " TEID_FMT " num bearers ctx %u",
      response_p->teid, response_p->bearer_contexts.num_bearer_context);
