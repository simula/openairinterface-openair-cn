  ...
  enum disp_action *act_p)
{
   ...
   */
  CHECK_FCT (fd_msg_search_avp (qry_p, s6a_fd_cnf.dataobj_s6a_origin_host, &origin_host_p));
   ...
   */
  CHECK_FCT (fd_msg_search_avp (qry_p, s6a_fd_cnf.dataobj_s6a_origin_realm, &origin_realm_p));
   ...
   */
  CHECK_FCT (fd_msg_search_avp (qry_p, s6a_fd_cnf.dataobj_s6a_cancellation_type, &avp_p));
  ...
  if (avp_p) {
      ...
      if (hdr_p->avp_value->u32 == SUBSCRIPTION_WITHDRAWL) {
        ...
        OAILOG_ERROR (LOG_S6A, "S6A_CANCEL_LOCATION cancellation type %d not supported \n", hdr_p->avp_value->u32);
        result_code = ER_DIAMETER_INVALID_AVP_VALUE;
      }
      goto out;
    } else {
...
out:
   ...
   */
  CHECK_FCT (fd_msg_search_avp (qry_p, s6a_fd_cnf.dataobj_s6a_auth_session_state, &avp_p));
  CHECK_FCT (fd_msg_avp_hdr (avp_p, &hdr_p));
   ...
   */
  CHECK_FCT (s6a_add_result_code (ans_p, failed_avp_p, result_code, experimental));
