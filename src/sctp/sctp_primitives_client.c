  ...
  sctp_data_t * sctp_data_p)
{
  ...
  while (repeat == 1) {
    ...
    if (ret < 0) {
      OAILOG_ERROR (LOG_SCTP, "[SD %d] Poll has failed (%d:%s)\n", sd, errno, strerror (errno));
      return errno;
    } else if (ret == 0) {
      /*
      ...
      if (n < 0) {
         ...
         */
        OAILOG_ERROR (LOG_SCTP, "[SD %d] An error occured during read (%d:%s)\n", sd, errno, strerror (errno));
        return 0;
      }
